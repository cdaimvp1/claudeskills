...
- A lite supplier-risk read (per the supplier-risk framework inlined in the lilly-brand-assets foundation at `/mnt/skills/user/lilly-brand-assets-1c344a/references/supplier-risk.md`) for leverage/context
...
## Integration
- **Consumes:** lilly-contract-review (playbook, SME matrix, a review's findings), negotiation-playbook-learning (supplier history and difficulty), legal-negotiation-prep or commercial-negotiation-prep (the briefing to practice against), the supplier-risk framework inlined in the lilly-brand-assets foundation at `/mnt/skills/user/lilly-brand-assets-1c344a/references/supplier-risk.md` (leverage context), the MSA + prior/expiring WOs for existing suppliers. For Internal Executive Challenge mode: the same playbook and supplier-risk framework (for approval-threshold and sole-source grounding), plus any business case, RFP/evaluation results, and approval-matrix/governance documents the user supplies.
