...
ROW form: each of its Cost-Driver Assumption Ledger rows IS a fact and carries its own source columns (`source`, `source_date`, `confidence`).
...
## Changelog
- **v1.3 (July 30, 2026):** Removed the optional standalone dashboard (the six-tab skeleton, `references/dashboard-canonical.md`, and `examples/should_cost_canonical_dashboard.jsx`, all deleted). It was a secondary, hand-authored-per-run visual with no deterministic builder; the XLSX + narrative + ledger were always the primary deliverable and stand alone unchanged. Its reconciliation invariants (Total_base foots to components, quadrature range ordering, gap-attribution summing) were already independently enforced in `should_cost_generator.py`'s `ReconciliationError`, so nothing analytical was lost. If a rendered visual is wanted, route to a locked hub dashboard rather than rebuilding one here.
...
- *Suite-wide guardrails note:* the shared G1-G13 execution guardrails (foundation reference) apply to every run; see GLOBAL OPERATING RULES Rule 9.
...
**Rule 2b: Role-family/driver-family deduplication and dated benchmark cache (per Execution Guardrails G7, F3).** The 3-search effort floor above is a floor, not a target, and this rule does not lower it; it removes duplicated search effort, not search. **This is a cost-avoidance mechanism, not a depth cap:** it targets redundant search/token spend on near-duplicate drivers that genuinely share a market, never a reduction in how much evidence any single distinct driver needs. A driver that does not genuinely share a market with anything else still gets its own full research pass to the same effort floor and evidence count it would need if clustering did not exist.
...
- **Illustrative savings (a worked example, not a promise).** A should-cost model that prices the same "Mid-level QA labor, nearshore" role across 6 BOM components/scenarios is one family, needing roughly 3-5 searches instead of 6 x 3-5 = 18-30: about an 80-85% cut in redundant search/token spend, with the same corroborating-source count still required before HIGH confidence is granted to that family. A model where every driver is genuinely distinct (the common mixed-BOM case) clusters into families of one and still runs the full per-driver search count either way, which is correct: there is nothing duplicated to avoid paying for. On a same-category re-run inside the cache window (below), a family with a fresh `fetched_date` recalls instead of re-sourcing, except FX and spot-commodity inputs, which are never cached (see the exception below) because their volatility is the risk the model is supposed to surface, not hide.
...
- No standalone dashboard is produced by this skill (removed 2026-07-30; see the Changelog). The XLSX, narrative, and ledger above are the complete, primary deliverable. If a rendered visual is wanted, route to a locked hub dashboard rather than building a second, redundant one here.
...
**Determinism:** the cost stack and its sources are shown; Total_base is the visible sum of component bases, and the total range is produced by the fixed Aggregation Method (quadrature with stated correlation grouping), reconciled on every run via the Pre-Delivery Self-Test (`should_cost_generator.py`'s `ReconciliationError` enforces this in code, independent of any rendering choice).
