...
- Optional `should_cost_dashboard.jsx` -- Magazine-style dashboard of the cost stack, built to the FIXED canonical tab skeleton (inlined below under "Dashboard canonical tab skeleton"). When the user wants a visual. Clone `examples/should_cost_canonical_dashboard.jsx` (the canonical reference implementation; full spec in `references/dashboard-canonical.md`). Do NOT hand-author JSX/React or CSS: your only job is the data object; the shipped, locked engine renders every tab; do not redesign the structure per run. **Graceful degradation:** the XLSX + narrative + ledger are the primary deliverable and stand alone. The dashboard is rendered via the `visualize:show_widget` primitive; if that primitive is unavailable, do not fail, deliver the workbook, the narrative, and the ledger, and tell the user the interactive view was skipped because the visualizer was not available.
