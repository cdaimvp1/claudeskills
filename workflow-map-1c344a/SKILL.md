...
- The cross-skill guardrails G1-G12 are summarized inline below, so this skill is self-sufficient. When the shared `lilly-brand-assets-1c344a` foundation (v10.0+) is installed, its `execution-guardrails` text is the fuller canonical version; defer to it when present, but never hard-depend on it.
...
- **Changelog:**
  ...
  - Suite-wide guardrails note: the cross-skill guardrails G1-G12 (not a per-skill version) are summarized inline in Operating Rule 9.
