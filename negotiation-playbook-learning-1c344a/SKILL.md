...
- **Suite-wide guardrails note (v8.2, May 2026):** Execution guardrails (G1-G13) are defined suite-wide in the shared foundation: tool-selection rules, mandatory gate checks, definition tracing, data-model-first for dashboards, pass-artifact enforcement, anti-collapse signal, and pre-delivery self-tests. This is a foundation-level note, not a per-skill version of this skill.
...
## Building the outcome JSON artifacts

```bash
python outcome_dataset_generator.py <records.json> <out_dataset.json> [YYYY-MM-DD]
python outcome_dataset_selftest.py                 # 29 assertions
```

`negotiation_outcome.json` and `outcome_dataset.json` are BUILT, not hand-assembled.
`outcome_summary.md` stays prose: a readable narrative is correctly model-authored.

The generator does not reimplement any arithmetic. `outcome_partition()` and
`difficulty_score()` in the vendored kernel already compute and validate every figure these
files carry, including the sum-to-1.0 integrity check, so this is serialization plus
assertion (G11).

**It refuses rather than emitting a dataset that quietly misreports.**

| refusal | why |
|---|---|
| an outcome code outside the eleven | an unrecognised code would be silently dropped from every rate |
| two records sharing a `dedup_key` | the schema says a repeat capture is an UPDATE, not a second outcome. Double-counting one negotiation biases every rate, partition and difficulty rollup |
| a stated `outcome_distribution` contradicting the actual rows | the summary is a rollup of the positions; when they disagree the positions win |
| partition rates not summing to 1.0 | the kernel's `PartitionError`; a failure means an outcome was miscounted |

**Zero applicable positions reports NEEDS_INPUT, never 0.** When every position is
`NOT_APPLICABLE` there is no denominator, so both the partition and the difficulty score
are labelled NEEDS_INPUT with the reason stated. Rates of 0.0 would read as "Lilly
prevailed on nothing" and a difficulty of 0 as "this negotiation was easy", when in fact
