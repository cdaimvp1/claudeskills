...
- The full guardrail set (G1-G10) is **inlined below** in this rule, so this skill is self-sufficient. When the shared `lilly-brand-assets-1c344a` foundation (v10.0+) is installed, its `references/execution-guardrails.md` is the fuller, canonical text; defer to it when present, but never hard-depend on it.
