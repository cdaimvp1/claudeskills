...
- **Changelog:**
  - v3.8 (July 2026): **F1: deterministic clause segmentation and generator-built findings ledger / redline instruction set.** New Phase 0A.5 ("Deterministic Clause Segmentation, Stage 0") runs once before Pass 1 and builds `CLAUSE_REGISTER` by calling the new vendored `contract_segmenter.py`; Passes 1 through 4 now read clauses from this one register by `clause_id` instead of each independently re-parsing the raw contract text or .docx XML, and Pass 1 additionally VERIFIES the register's segmentation against the source rather than trusting it (`verify_register_against_source()`). **All four passes keep their full existing scope, judgment, and outputs unchanged** -- this is a change to where each pass reads its input text from, not to what any pass decides. New vendored `contract_review_generator.py` mechanically produces the findings ledger (`.json`, schema unchanged from v3.4) and a validated redline instruction set from the already-specified `PASS_4_PREP` data object (`references/pass-artifacts.md`, unchanged), replacing narrative re-assembly at Step 5 with a generator that refuses (raises) rather than emits when a citation does not resolve against `CLAUSE_REGISTER`, the hard-stop count disagrees with the findings, a Hard Stop/SME finding was silently downgraded to a lower-visibility comment, or the Rule 12 calculation table does not foot to the stated score. `Comment`-application-mode redlines are now built directly by the generator using python-docx's native comment API; `Auto`/`Walk-through` body-text tracked-change mechanics are unchanged (see `contract_review_generator.py`'s own docstring for why that specific piece remains the existing model + `docx`-skill path rather than a newly-authored, unproven OOXML writer). The Protection Score formula, the deduction magnitudes, Rule 12 itself, and every guardrail, Hard Stop list, and SME-routing rule are UNCHANGED; `deduction_score()` (item C1) remains explicitly held (`references/dashboard-canonical.md` HELD #114) and is not built or invoked by this change. Verified against a golden fixture (`_audit/golden-fixture/`): every one of the 36 real answer-key finding IDs, all citations, and the hard-stop count survive the new segmentation-and-generator pipeline with zero drops and zero drift.
...
4. **Findings ledger (.json)** (Full review and Dashboard only; offer it for Redline only) -- a machine-readable sidecar that mirrors the same finding objects the dashboard and review summary use, so downstream skills (negotiation-playbook-learning, theos-field-guide) can ingest the review without re-parsing a DOCX. It is generated from the already-built `PASS_4_PREP` data object by calling `contract_review_generator.build_findings_ledger()` (vendored in this skill's own directory), not authored separately, so it cannot drift from the human artifacts. `build_findings_ledger()` calls `validate_register()` first, which refuses (raises) rather than emitting a ledger if any finding is missing a required field, any citation does not resolve against `CLAUSE_REGISTER`, the hard-stop count does not match the findings actually marked Hard Stop, or the Rule 12 calculation table does not foot to the stated score -- so an internally-inconsistent review is caught before a deliverable is produced, not after. Schema per finding: `{id, severity (HIGH/MEDIUM/LOW/HARD_STOP), title, category, coverage_status (Standalone/Covered/Confirm/Gap), where, msa_reference, verified (true/false), impact, recommended_action, deduction}`, plus a header block `{supplier, document, governing_agreement, risk_score, hard_stops, covered_count, confirm_count, gap_count, as_of_date}`. The `risk_score` and the per-finding `deduction` values MUST equal the values in the Risk Heatmap calculation table (single source of truth); if they do not foot, fix the calculation, not the ledger. This is a local artifact only: it is never auto-sent or written to any M365 location.
...
**Phase 0A.5: Deterministic Clause Segmentation (Stage 0, runs ONCE before Pass 1)**

Before Pass 1 begins, build the `CLAUSE_REGISTER`: a single structured clause register spanning every document in the family (the document under review plus every governing document read in Phase 0A). Build it by calling `contract_segmenter.build_register()` (vendored in this skill's own directory, `lilly-contract-review-1c344a/contract_segmenter.py`) exactly once per review, passing each document's already-extracted text or, for a .docx read via `unpack.py` per Execution Guardrail G1, its `word/document.xml` (plus `word/comments.xml` and `word/people.xml` when present) to `segment_docx_parts()` so tracked-change status, authorship, and comment anchors are captured per G3.

`CLAUSE_REGISTER` replaces raw re-reading as the input to every subsequent pass: **Passes 1 through 4 read clauses from `CLAUSE_REGISTER` by `clause_id` (e.g. `WO-10:7.1`, `MSA:23.1`) instead of re-parsing the raw contract text or .docx XML a second, third, and fourth time.** This is a pure efficiency change to HOW each pass reads its input, not a change to WHAT each pass decides: Pass 1 through Pass 4 keep their full existing scope, judgment, and outputs exactly as specified below. Determinism NARROWS the input each pass reads; it never decides a finding, a severity, or a coverage status, and it never drops a clause -- `contract_segmenter.py` captures a clause it cannot cleanly number as an `unstructured` fallback entry rather than silently omitting it, so no source text falls outside the register's coverage.

**Pass 1 verifies the register rather than trusting it blindly.** Because a segmentation defect that silently drops or mis-splits a clause would be the worst possible bug in this design (the output would look complete while missing something), Pass 1's Structural Scan step below calls `contract_segmenter.verify_register_against_source()` for the document under review and states the result (clause count expected vs. found, any missing or extra section numbers) in `PASS_1_STRUCTURE`. If the verification finds a mismatch, STOP and re-segment or fall back to reading the raw text for the affected document before proceeding; do not proceed on a register known to disagree with its own source.

**Citation resolution.** Every finding's contract-language citation (Rule 2) should resolve against `CLAUSE_REGISTER` via `contract_segmenter.resolve_reference()` before the finding is finalized in Pass 4. A citation that does not resolve is either a typo in the citation or a segmentation gap; either way, verify it against the raw document rather than emitting an unresolvable citation.

**Scope note.** `CLAUSE_REGISTER` structures the CONTRACT text only (clause identity, heading, verbatim text, tracked-change status). It does not narrow or index the judgment corpora (`playbook.md`, `vendor-tactics.md`, etc.); that is a separate, already-built and independently self-tested utility, `retrieval_index.py` in this same directory, which is not wired into this workflow by this change.
...
**Pass structure.** Every pass below reads from `CLAUSE_REGISTER` (Phase 0A.5) by `clause_id`, not by re-parsing the raw contract text or .docx XML; each pass loads only the clauses relevant to its own job (Pass 2 pulls the full register plus the definition inventory, Pass 3 pulls clauses touching pricing/tactics/pharma categories, etc.), and fetches a clause's full text on demand by `clause_id` when reasoning about it specifically. This changes WHERE each pass gets its text from; it changes nothing about what each pass does with it:
- **Pass 1 - Structural Scan:** Document classification, party identification, commercial terms extraction, deliverable inventory, timeline mapping. Do NOT apply playbook yet - just understand the document. Also runs `verify_register_against_source()` (Phase 0A.5) and records the result: this pass now VERIFIES `CLAUSE_REGISTER`'s segmentation rather than performing that segmentation itself.
...
- **Pass 4 - Output Quality Assurance (for complex reviews):** Re-read all findings against the governing document landscape. Verify that every finding is accurate given MSA protections. Remove false positives (issues the MSA already covers that were flagged in Pass 2 before full MSA context was absorbed). Ensure every finding has: (a) specific contract reference, (b) playbook or regulatory citation, (c) recommended action, (d) impact assessment. Also resolves every finding's citation against `CLAUSE_REGISTER` per Phase 0A.5 before the finding is finalized, and this is where the `PASS_4_PREP` data object (`references/pass-artifacts.md`) becomes the persistent findings register that Step 5 generates deliverables from, rather than a working note that is re-narrated at output time.
...
**First, build the redline instruction set from `PASS_4_PREP`, deterministically, by calling `contract_review_generator.build_redline_instructions()`** (vendored in this skill's own directory). This produces one instruction per finding (clause anchor, comment classification, comment text, whether it must be stripped before supplier transmission), validated against `CLAUSE_REGISTER` so no instruction can cite a clause the document does not contain. Step 5A then executes this instruction set rather than the model re-deciding "what goes where" a second time from narrative memory; the instruction set is the single source of truth for every comment and Hard Stop/SME flag this step produces.

**In `Comment` application mode** (the default whenever the document already carries supplier changes), call `contract_review_generator.build_redlined_docx()` to render the redline directly: it reconstructs the document from `CLAUSE_REGISTER` and attaches a native Word comment (via python-docx's `Document.add_comment()`, a real comments part with correct relationships, not hand-rolled XML) at each clause the instruction set names, always including every 🔴 Hard Stop and 🔵 SME Escalation comment regardless of what else the mode does (the HARD RULE below). A finding whose citation resolves to a governing document rather than a clause in the document under review (a Rule 9a covered-absence finding, for example) is rendered under a "General Findings" heading rather than silently omitted.

**In `Auto` and `Walk-through` application modes**, the body-text mechanics (accepting, rejecting, or inserting a supplier's tracked change) remain the existing path: use the `docx` skill on the document as today. This step's comment layer -- what `build_redline_instructions()` produced -- is still the authoritative source for every comment those two modes insert; only the body-edit mechanic itself is unchanged. (Scope note: `contract_review_generator.py` does not author `<w:ins>`/`<w:del>` tracked-change XML; see the module's own docstring for why that specific piece was left as the existing model + `docx`-skill path rather than guessed at.)

If no file-generation capability is available in the current surface, provide the complete tracked-change and comment set as a structured list (location, change, comment, basis) so the user can apply it in Word manually, and say the DOCX could not be generated. Specifically:
