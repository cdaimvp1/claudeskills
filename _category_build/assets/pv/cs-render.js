...
var CS_NAV = [
  ...
  ['trend',    'Trend & Change',    [['trend', 'Trend'], ['rational', 'Tail & Rationalization']]],
...
function scOverview() {
  ...
  /* The exposure half of the old Contract Opportunities panel: renewal timing and
     contract hygiene, which belong beside Renewal Exposure rather than on a
     consolidation screen. The consolidation half stayed on Tail & Rationalization. */
  var xs = csContractOppSplit(d);
  if (xs.exposure.length) h += csCard('Contract exposure', csContractOpps(xs.exposure),
      { icon: 'cal', role: 'emph', sub: xs.exposure.length + ' agreements at risk', conf: 'Moderate' });
  return h;
}
...
function scTrend() {
  ...
  /* The old strip led with FY24-to-FY25 and FY23-to-FY24, which the chart below
     already draws. Those two moved ONTO the chart as year-over-year deltas, and
     the strip now carries what the chart cannot say: how much of the growth was
     price rather than volume, how concentrated the growth was, and whether the
     supplier base grew with the spend. */
  var rv = d.rateVolume, sw = (d.swing || []).slice().sort(function (a, b) {
    return Math.abs(b.delta || 0) - Math.abs(a.delta || 0); });
  var top3 = sw.slice(0, 3).reduce(function (a, x) { return a + Math.abs(x.delta || 0); }, 0);
  var swTotal = sw.reduce(function (a, x) { return a + Math.abs(x.delta || 0); }, 0);
  var perVendorNow = (m.s25 && m.vendors) ? m.s25 / m.vendors : null;
  var perVendorPrior = (m.s24 && m.vendorsPrior) ? m.s24 / m.vendorsPrior : null;
  var pvDelta = (perVendorNow && perVendorPrior)
    ? ((perVendorNow - perVendorPrior) / perVendorPrior) * 100 : null;

  h += '<div class="cs-kpirow">'
    + csKpi('Price vs volume', rv && rv.total
        ? csPct((rv.rate / rv.total) * 100, 0) + ' price'
        : '--',
        rv && rv.total ? csUsd(rv.rate) + ' of ' + csUsd(rv.total) + ' was rate, not quantity'
                       : 'needs a rate and quantity split', 'plum')
    + csKpi('Growth concentration', swTotal ? csPct((top3 / swTotal) * 100, 0) : '--',
        swTotal ? 'of the movement came from three suppliers' : 'needs swing[]', 'emph')
    + csKpi('Spend per supplier', perVendorNow ? csUsd(Math.round(perVendorNow)) : '--',
        pvDelta != null
          ? (pvDelta > 0 ? '+' : '') + csPct(pvDelta, 0) + ' vs FY24, so spend grew faster than the base'
          : 'FY25 average')
    + csKpi('Supplier base', (m.newVendorsNet != null ? (m.newVendorsNet > 0 ? '+' : '') + csNum(m.newVendorsNet) : '--'),
        m.vendorsPrior ? csNum(m.vendorsPrior) + ' to ' + csNum(m.vendors) + ' active' : 'net movement', 'teal')
    + '</div>';
...
function csTrendSvg(series, opts) {
  ...
  var cols = series.map(function (p, i) {
    ...
    /* The year-over-year step, drawn where the comparison actually happens.
       This is what the two removed KPI cards used to say. */
    var prev = i > 0 ? (series[i - 1].total != null ? series[i - 1].total : series[i - 1].v) : null;
    var yoy = (prev && top) ? ((top - prev) / prev) * 100 : null;
    var yoyTag = yoy == null ? ''
      : '<div class="cs-fc-yoy ' + (yoy >= 0 ? 'up' : 'dn') + '">'
        + (yoy > 0 ? '+' : '') + csPct(yoy, 1) + '</div>';
...
function csFragMap(d) {
  ...
  var W = 600, H = 340, PADL = 70, PADB = 50, PADT = 24, PADR = 24;
...
function csRiskMatrix(d, rows) {
  ...
  /* Same bubble scale as the Kraljic plot, so a circle reads the same size for
     the same weight across both panels. Severity drives the radius: a
     high/high risk is a big circle, a low/low one is small. */
  var dots = r.map(function (x, i) {
    ...
    var weight = (csLI(x.l) * csLI(x.i)) / (3.5 * 3.5);
    var rr = csBubbleR(weight);
    return '<circle cx="' + cx.toFixed(1) + '" cy="' + cy.toFixed(1) + '" r="' + rr.toFixed(1)
      + '" class="' + (hot ? 'cs-rk-hot' : 'cs-rk') + '">'
...
function csRenewalMatrix(d, sel) {
  ...
  var W = 600, H = 380, PADL = 78, PADR = 22, PADT = 44, PADB = 56;
  var pw = W - PADL - PADR, ph = H - PADT - PADB;
...
/* Shared bubble geometry. Area scales with the value, which is what the eye
   actually reads, and both ends are clamped so a small bubble stays legible and
   a large one does not swallow its neighbours. Used by the Kraljic plot and the
   risk heatmap so the two are comparable at a glance. */
var CS_R_MIN = 10, CS_R_MAX = 26;
function csBubbleR(frac) {
  var f = Math.max(0, Math.min(1, frac || 0));
  return CS_R_MIN + (CS_R_MAX - CS_R_MIN) * Math.sqrt(f);
}
/* Labels are placed after every bubble is positioned, then pushed apart on the
   vertical until none overlaps. Previously each label sat a fixed distance under
   its own bubble, which put it underneath the next bubble down. */
function csPlaceLabels(items, minGap) {
  var gap = minGap || 13;
...
function csKraljicPlot(d) {
  ...
  var W = 600, H = 340, PADL = 74, PADR = 92, PADT = 28, PADB = 48;
  var pw = W - PADL - PADR, ph = H - PADT - PADB;
  var maxSize = segs.reduce(function (a, s) {
    return Math.max(a, CS_SEG_SIZE === 'vendors' ? (s.vc || 0) : (s.tot || 0));
  }, 0) || 1;
  /* One scale for every bubble chart on this dashboard, so a circle means the
     same thing whichever panel it is in. See CS_R_MIN / CS_R_MAX. */

  var avgX = pts.reduce(function (a, p) { return a + p.x; }, 0) / pts.length;
  var avgY = pts.reduce(function (a, p) { return a + p.y; }, 0) / pts.length;

  /* Position every bubble first, then place the labels off to one side and
     push them apart vertically. Labels used to sit a fixed distance under their
     own bubble, which parked them underneath the next bubble down. Each label
     leans away from the centre so it never crosses the plot. */
  var placed = pts.map(function (p) {
    ...
    var r = csBubbleR(Math.max(0, v) / (s.market ? 1 : maxSize));
    /* Clamp the centre by the radius so a bubble at the extreme of either axis
       is not sliced by the plot edge. The position still reads correctly: the
       clamp only bites where a circle would otherwise hang outside the frame. */
    var cx = Math.max(PADL + r, Math.min(PADL + pw - r, PADL + p.x * pw));
    var cy = Math.max(PADT + r, Math.min(PADT + ph - r, PADT + ph - p.y * ph));
    var right = p.x < 0.58;                       // lean away from the far edge
    return { p: p, s: s, r: r, cx: cx, cy: cy, right: right,
             lx: right ? cx + r + 13 : cx - r - 13, y: cy + 4 };
  });
  csPlaceLabels(placed, 14);

  var dots = placed.map(function (o) {
    var s = o.s, p = o.p, q = csKraljicQuad(p);
    ...
    var leader = (Math.abs(o.y - (o.cy + 4)) > 6)
      ? '<line x1="' + (o.right ? o.cx + o.r : o.cx - o.r).toFixed(1) + '" y1="' + o.cy.toFixed(1)
        + '" x2="' + o.lx.toFixed(1) + '" y2="' + (o.y - 4).toFixed(1) + '" class="cs-kdlead"/>' : '';
    return '<g class="cs-kdot' + (on ? ' on' : '') + (dim ? ' dim' : '') + '" onclick="csSegPick(\''
      + csEsc(String(s.key).replace(/'/g, "\\'")) + '\')" role="button" tabindex="0">'
      + leader
      + '<circle cx="' + o.cx.toFixed(1) + '" cy="' + o.cy.toFixed(1) + '" r="' + o.r.toFixed(1)
      + '" fill="' + tone + '" fill-opacity="0.42" stroke="' + tone + '" stroke-width="1.5">'
      + '<title>' + csEsc(s.market
          ? s.key + ' · ' + q + ' · ' + s.headline
          : s.key + ' · ' + q + ' · ' + csUsd(s.tot)
            + (s.vc ? ' · ' + csNum(s.vc) + ' vendors · ' + csUsd(Math.round(p.perVendor)) + ' average each' : ''))
      + '</title></circle>'
      + '<text x="' + o.lx.toFixed(1) + '" y="' + o.y.toFixed(1) + '" class="cs-kdlab"'
      + (o.right ? '' : ' text-anchor="end"') + '>'
      + csEsc(String(s.key).slice(0, 24)) + '</text></g>';
  }).join('');
...
function csKraljicRead(d, pts) {
  ...
  /* Only the segmented finding stays in the card: it is the one part that
     changes when the toggle moves. The posture and the axis definition are
     constant, so they sit in their own panel below the row. */
  return '<div class="cs-read"><div class="cs-read-d">' + lead + '</div></div>';
}

/* The posture and the axis definition do not move when the toggle moves, so
   they are a panel of their own rather than dead weight inside the chart card. */
function csPositionNotes(d) {
  var n = d.narr || {};
  var impl = n.kraljicImpl
    ? '<div class="cs-pn-block"><div class="cs-lab2">What that means for how we buy</div>'
      + csNarr(n.kraljicImpl) + '</div>' : '';
  var axes = '<div class="cs-pn-block"><div class="cs-lab2">How the axes are derived</div>'
    + '<div class="cs-pn-d"><b>Profit impact</b> is share of category spend. '
    + '<b>Supply risk</b> is average spend per vendor in the segment, log-scaled: where the money sits in '
    + 'a few large relationships the incumbent is hard to replace, and where it is spread thin across many '
    + 'small ones it is not. A raw vendor count was the obvious proxy and it was wrong, because it made '
    + 'Scientific Research, which has the most vendors in the portfolio, look like the safest segment. '
    + 'Both axes are read off held data; neither is a judgement.</div></div>';
  return '<div class="cs-pnotes">' + impl + axes + '</div>';
}
...
function scMarket() {
  ...
  h += csCard('Reading the position', csPositionNotes(d),
      { icon: 'doc', sub: 'constant across every segmentation', conf: csConfFor(d, 'narr') });
  ...
  var rk = csRisksFor(d);
  h += '<div class="cs-row2">'
    + csCard('Risk heatmap', csRiskMatrix(d, rk.rows),
...
/* Derive a short subject from the headline when the data does not carry one. */
var CS_SUBJECTS = [
  [/renewal|bump|uplift/i, 'Renewal pricing'],
  [/after signature|unexpected|consumption/i, 'Unbudgeted AI charges'],
  [/megavendor|installed base|list price/i, 'Megavendor list increases'],
  [/regulat|ai act|penalt/i, 'AI regulation in contracts'],
  [/insurance|indemnit/i, 'Indemnity and insurance gap'],
  [/consolidat/i, 'Vendor consolidation']
];
function csSubjectOf(x) {
  var hay = (x.t || '') + ' ' + (x.d || '');
  for (var i = 0; i < CS_SUBJECTS.length; i++) if (CS_SUBJECTS[i][0].test(hay)) return CS_SUBJECTS[i][1];
  return String(x.t || '').split(/[,:.]/)[0].slice(0, 34);
}
function csMarketIntelSplit(mk) {
  /* Closed, a headline like "The routine renewal bump is gone" tells you nothing
     about what is inside. Closed rows show the SUBJECT; the headline is the
...
/* ===========================================================================
   TAIL & RATIONALIZATION — the merged screen (option D)

   Overlap & Consolidation asked "are we buying the same capability twice".
   Tail Consolidation Opportunities asked "are we buying small things from too
   many people". Both end in the same move, so they are one screen answering one
   question: where are we buying the same thing more than once, and what is it
   worth to stop.

   What did NOT merge, deliberately: Utilization/Shelfware answers a different
   question (paying for what we do not use) and Action Matrix closes the tab with
   every supplier dispositioned. Contract Opportunities SPLIT: the one
   item routing off-contract spend onto agreements that already exist is
   consolidation and stays here; the four that are renewal timing or contract
   hygiene are exposure and moved to Overview beside Renewal Exposure.
   =========================================================================== */

/* Three sources, one shape. Kind is the distinction the two old panels carried:
   capability = the same thing bought twice, sprawl = small things bought from
   too many people, volume = same supplier, fewer contracts, better tier. */
function csConsolidationOpps(d) {
  var out = [];

  (d.subcats || []).forEach(function (sc) {
    (sc.opps || []).forEach(function (o) {
      var from = o.from || 0, to = o.to || 0;
      out.push({
        title: o.t, detail: o.d,
        kind: o.kind === 'volume' ? 'volume' : (from >= 20 ? 'sprawl' : 'capability'),
        value: o.value == null ? null : o.value,
        from: from, to: to, removed: Math.max(0, from - to),
        vehicle: o.vehicle || '', where: sc.n,
        effort: o.kind === 'volume' ? 'Medium' : (from >= 20 ? 'High' : (from <= 4 ? 'Low' : 'Medium'))
      });
    });
  });

  (d.tailOpps || []).forEach(function (t) {
    var n = (t.vendors || []).length;
    out.push({
      title: t.group, detail: t.action,
      kind: 'sprawl', value: t.saving == null ? null : t.saving,
      from: n, to: 1, removed: Math.max(0, n - 1),
      vehicle: '', where: 'tail', effort: t.effort || 'Medium',
      combined: t.combined, vendors: t.vendors
    });
  });

  /* Capability overlaps carry no saving of their own; they are the evidence
     behind the capability opportunities above, and drive the coverage map. */
  return out.sort(function (a, b) { return (b.value || 0) - (a.value || 0); });
}

var CS_EFFORT_X = { Low: 0.18, Medium: 0.5, High: 0.84 };
var CS_KIND = {
  capability: { label: 'Capability', tone: 'var(--plum)', cls: 'p-p' },
  sprawl:     { label: 'Supplier sprawl', tone: 'var(--emph)', cls: 'p-e' },
  volume:     { label: 'Volume', tone: 'var(--teal)', cls: 'p-t' }
};

/* Value against effort. The ranked list says what things are worth; it does not
   say what to do first. This does, and it exposes something the list hides: the
   biggest supplier-count reductions tend to be the hardest and least valuable. */
function csConsolScatter(d) {
  var ops = csConsolidationOpps(d).filter(function (o) { return o.value != null; });
  if (!ops.length) return csGap('Where to start: value against effort.', 'opportunity records with a value and an effort');
  var W = 600, H = 320, PADL = 66, PADR = 26, PADT = 28, PADB = 50;
  var pw = W - PADL - PADR, ph = H - PADT - PADB;
  var maxV = ops.reduce(function (a, o) { return Math.max(a, o.value || 0); }, 0) || 1;
  var maxR = ops.reduce(function (a, o) { return Math.max(a, o.removed || 0); }, 0) || 1;

  var placed = ops.map(function (o) {
    var r = csBubbleR((o.removed || 0) / maxR);
    var cx = Math.max(PADL + r, Math.min(PADL + pw - r,
      PADL + (CS_EFFORT_X[o.effort] != null ? CS_EFFORT_X[o.effort] : 0.5) * pw));
    var cy = Math.max(PADT + r, Math.min(PADT + ph - r, PADT + ph - ((o.value || 0) / maxV) * ph));
    var right = cx < PADL + pw * 0.55;
    return { o: o, cx: cx, cy: cy, r: r, right: right,
             lx: right ? cx + r + 13 : cx - r - 13, y: cy + 4 };
  });
  csPlaceLabels(placed, 14);

  var dots = placed.map(function (p) {
    var k = CS_KIND[p.o.kind] || CS_KIND.capability;
    var leader = (Math.abs(p.y - (p.cy + 4)) > 6)
      ? '<line x1="' + (p.right ? p.cx + p.r : p.cx - p.r).toFixed(1) + '" y1="' + p.cy.toFixed(1)
        + '" x2="' + p.lx.toFixed(1) + '" y2="' + (p.y - 4).toFixed(1) + '" class="cs-kdlead"/>' : '';
    return '<g class="cs-kdot">' + leader
      + '<circle cx="' + p.cx.toFixed(1) + '" cy="' + p.cy.toFixed(1) + '" r="' + p.r.toFixed(1)
      + '" fill="' + k.tone + '" fill-opacity="0.42" stroke="' + k.tone + '" stroke-width="1.5"><title>' + csEsc(p.o.title + ' · ' + k.label + ' · '
      + csUsd(p.o.value) + ' · ' + p.o.effort + ' effort'
      + (p.o.removed ? ' · ' + csNum(p.o.removed) + ' suppliers removed' : '')) + '</title></circle>'
      + '<text x="' + p.lx.toFixed(1) + '" y="' + p.y.toFixed(1) + '" class="cs-kdlab"'
      + (p.right ? '' : ' text-anchor="end"') + '>'
      + csEsc(String(p.o.title).slice(0, 30)) + ' · ' + csUsd(p.o.value) + '</text></g>';
  }).join('');

  return '<div class="cs-pwrap"><svg viewBox="0 0 ' + W + ' ' + H + '" class="cs-kplot" role="img" '
    + 'aria-label="Consolidation value against effort">'
    + '<line x1="' + (PADL + pw / 2) + '" y1="' + PADT + '" x2="' + (PADL + pw / 2) + '" y2="' + (PADT + ph) + '" class="cs-p80h"/>'
    + '<line x1="' + PADL + '" y1="' + (PADT + ph / 2) + '" x2="' + (W - PADR) + '" y2="' + (PADT + ph / 2) + '" class="cs-p80h"/>'
    + csQuadLabel(PADL + 4, PADT - 12, 'start', ['Do first', 'high value, low effort'])
    + csQuadLabel(W - PADR - 2, PADT - 12, 'end', ['Plan properly', 'high value, high effort'])
    + csQuadLabel(PADL + 4, PADT + ph + 16, 'start', ['Sweep up', 'low value, low effort'])
    + csQuadLabel(W - PADR - 2, PADT + ph + 16, 'end', ['Leave it', 'low value, high effort'])
    + '<line x1="' + PADL + '" y1="' + (PADT + ph) + '" x2="' + (W - PADR) + '" y2="' + (PADT + ph) + '" class="cs-pax"/>'
    + '<line x1="' + PADL + '" y1="' + PADT + '" x2="' + PADL + '" y2="' + (PADT + ph) + '" class="cs-pax"/>'
    + ['Low', 'Medium', 'High'].map(function (e) {
        return '<text x="' + (PADL + CS_EFFORT_X[e] * pw).toFixed(1) + '" y="' + (PADT + ph + 34)
          + '" class="cs-pyr" text-anchor="middle">' + e + '</text>';
      }).join('')
    + [0, 0.5, 1].map(function (t) {
        return '<text x="' + (PADL - 8) + '" y="' + (PADT + ph - t * ph + 4).toFixed(1) + '" class="cs-pyl">'
          + csUsd(maxV * t) + '</text>';
      }).join('')
    + '<text x="' + (PADL + pw / 2) + '" y="' + (H - 6) + '" class="cs-axt">Effort to execute</text>'
    + '<text x="14" y="' + (PADT + ph / 2) + '" class="cs-axt" transform="rotate(-90 14,' + (PADT + ph / 2) + ')">Value</text>'
    + dots + '</svg></div>'
    + '<div class="cs-fc-key"><span class="cs-k-cap">Capability</span><span class="cs-k-spr">Supplier sprawl</span>'
    + '<span class="cs-k-vol">Volume</span><span class="cs-k-size">bubble = suppliers removed</span></div>'
    + csConsolRead(d, ops);
}

function csConsolRead(d, ops) {
  var byKind = { capability: 0, sprawl: 0, volume: 0 };
  ops.forEach(function (o) { byKind[o.kind] = (byKind[o.kind] || 0) + (o.value || 0); });
  var hardBig = ops.filter(function (o) { return o.effort === 'High'; })
    .sort(function (a, b) { return (b.removed || 0) - (a.removed || 0); })[0];
  if (!hardBig) return '';
  return '<div class="cs-read"><div class="cs-read-d">'
    + 'The money is in <b>capability overlap</b> (' + csUsd(byKind.capability) + '), the effort is in '
    + '<b>supplier sprawl</b> (' + csUsd(byKind.sprawl) + '). '
    + 'The largest supplier-count reduction on the board, <b>' + csEsc(hardBig.title) + '</b> at '
    + csNum(hardBig.removed) + ' suppliers, is also high effort and worth only ' + csUsd(hardBig.value)
    + '. A ranked list hides that; the quadrant does not. Start top-left.'
    + '</div></div>';
}

/* The same set as a table, because the scatter says what first and the table
   says what exactly. */
function csConsolList(d) {
  var ops = csConsolidationOpps(d);
  if (!ops.length) return csGap('Consolidation opportunities.', 'per-subcategory opportunity records and tail groupings');
  var rows = ops.map(function (o) {
    var k = CS_KIND[o.kind] || CS_KIND.capability;
    return '<tr><td class="cs-l"><b>' + csEsc(o.title) + '</b>'
      + (o.where && o.where !== 'tail' ? '<span class="cs-sub2">' + csEsc(o.where) + '</span>' : '')
      + '</td>'
      + '<td class="cs-num"><span class="cs-pill cs-pill-' + (o.kind === 'sprawl' ? 'burnt' : o.kind === 'volume' ? 'teal' : 'plum')
      + '">' + k.label + '</span></td>'
      + '<td class="cs-num">' + (o.from ? csNum(o.from) + ' &rarr; ' + csNum(o.to) : 'same') + '</td>'
      + '<td class="cs-num">' + (o.value == null ? 'not sized' : csUsd(o.value)) + '</td>'
      + '<td class="cs-num">' + csEsc(o.effort) + '</td>'
      + '<td class="cs-why">' + csEsc(o.detail || '') + (o.vehicle ? ' <b>' + csEsc(o.vehicle) + '</b>' : '') + '</td></tr>';
  });
  return csTable(['Opportunity', 'Kind', 'Suppliers', 'Value', 'Effort', 'What to do'], rows);
}

function csConsolTotals(d) {
  var ops = csConsolidationOpps(d);
  var tot = ops.reduce(function (a, o) { return a + (o.value || 0); }, 0);
  var low = ops.filter(function (o) { return o.effort === 'Low'; })
    .reduce(function (a, o) { return a + (o.value || 0); }, 0);
  var byKind = { capability: 0, sprawl: 0, volume: 0 };
  ops.forEach(function (o) { byKind[o.kind] += (o.value || 0); });
  var max = Math.max(byKind.capability, byKind.sprawl, byKind.volume) || 1;
  var bar = function (k, label, tone) {
    return '<div class="cs-ck-row"><span>' + label + '</span>'
      + '<span class="cs-bar cs-bar-' + tone + '" style="width:100%"><i style="width:'
      + ((byKind[k] / max) * 100).toFixed(0) + '%"></i></span>'
      + '<b>' + csUsd(byKind[k]) + '</b></div>';
  };
  return '<div class="cs-split2">'
    + '<div class="cs-stat"><div class="cs-stat-v">' + csUsd(tot) + '</div>'
      + '<div class="cs-stat-l">identified</div>'
      + '<div class="cs-stat-s">' + ops.length + ' opportunities</div></div>'
    + '<div class="cs-stat"><div class="cs-stat-v">' + csUsd(low) + '</div>'
      + '<div class="cs-stat-l">low effort</div>'
      + '<div class="cs-stat-s">start here</div></div>'
    + '</div>'
    + '<div class="cs-ckinds"><div class="cs-lab2">By kind</div>'
    + bar('capability', 'Capability overlap', 'plum')
    + bar('sprawl', 'Supplier sprawl', 'emph')
    + bar('volume', 'Volume consolidation', 'teal') + '</div>';
}

/* ---- the merged screen ---- */
function scTailRational() {
  var d = csData(), n = d.narr || {};
  var h = '';

  h += '<div class="cs-row2">'
    + csCard('Where to start', csConsolScatter(d),
        { icon: 'target', role: 'solid', sub: 'value against effort', conf: csConfFor(d, 'savings') })
    + csCard('What it is worth', csConsolTotals(d),
        { icon: 'money', role: 'teal', sub: 'by kind', conf: csConfFor(d, 'savings') })
    + '</div>';

  h += csCard('Consolidation opportunities', csConsolList(d)
      + csNote('<b>Capability</b> is the same thing bought twice. <b>Supplier sprawl</b> is small things '
        + 'bought from too many people. <b>Volume</b> is the same supplier on fewer contracts at a better '
        + 'tier. The first two were separate panels until they were merged; they always ended in the '
        + 'same move.'),
      { icon: 'list', sub: 'ranked by value', conf: csConfFor(d, 'savings') });

  if (d.overlaps) h += csCard('Capability coverage', csOverlaps(d.overlaps),
      { icon: 'grid', sub: d.overlaps.length + ' capabilities served more than once', conf: 'Moderate' });
  ...
  h += '<div class="cs-row2">'
    + csCard('Utilization / shelfware', d.utilization ? csUtilization(d.utilization)
        ...
        { icon: 'warn', role: 'emph', sub: 'a different question: paying for what we do not use',
          conf: d.utilization ? 'Moderate' : 'Limited' })
  ...
  var split = csContractOppSplit(d);
  if (split.consol.length) {
    h += csCard('Off-contract spend to route', csContractOpps(split.consol)
        + csNote('These are consolidation: spend moving onto agreements that already exist. Renewal '
          + 'timing and contract hygiene are exposure rather than consolidation, so they sit on '
          + 'Overview beside Renewal Exposure.'),
        { icon: 'cal', sub: split.consol.length + ' to route onto existing vehicles', conf: 'Moderate' });
  }
  return h;
}

/* Marc: contract opportunities can stay here IF they are consolidation. Read
   against the data, only one of the five is: routing off-contract spend onto
   agreements that already exist. The other four are renewal timing and contract
   hygiene, which are exposure, not consolidation, so they go to Overview beside
   Renewal Exposure. Splitting beats moving all five in either direction. */
function csContractOppSplit(d) {
  var all = d.contractOpps || [];
  var isConsol = function (o) {
    return /catalogue|existing master|existing MSA|preferred-supplier|route through/i.test(o.action || '');
  };
  return { consol: all.filter(isConsol), exposure: all.filter(function (o) { return !isConsol(o); }) };
}
...
function csBody() {
  ...
  if (CS_TAB === 'trend')    return sub === 'rational' ? scTailRational() : scTrend();
  return scOverview();
}
function csRender() {
