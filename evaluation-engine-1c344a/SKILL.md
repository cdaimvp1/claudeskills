...
- **Changelog:**
  - v2.4 (July 2026): Removed the standalone Canonical Dashboard (the six-tab `visualize:show_widget` spec, `supplier_evaluation_ui.json` data model, and its `examples/evaluation_engine_canonical_dashboard.jsx` reference build) and this skill's own `evaluation_engine_canonical_dashboard.jsx` companion file. This skill's official data (panel scoring, ranking, dispersion, calibration, audit trail, readiness, and the sensitivity/award-scenario `modelDecision`) already renders in the locked rfx-hub-1c344a dashboard as one of its four feeder slices, labeled OFFICIAL. Five of the six removed tabs (Overview, Scoring Detail, Requirements Coverage, Risk, Sensitivity) mapped to content already in evaluation_report.docx (Sections 3-5, 7) or a CSV pipeline artifact; nothing there was lost. The sixth, the Award Scenario single-vs-split-award modeler, had no DOCX or CSV equivalent (it was new in v2.3 and purely a dashboard feature), so rather than drop that analysis, it was relocated into evaluation_report.docx Section 7 as a new "Award Scenario Comparison" subsection: the same eligibility gating and the same `weighted_score()` kernel call (allocation fractions as the weight set) now compute 2-3 static illustrative scenarios instead of a live client-side slider. Fixed several now-stale references elsewhere in this file (the Output Selection picker mapping, the Outputs table, the Reference Files manifest, the Risk Heatmap Output section, the Communication Outputs branding line, and the capability-adaptation deliverable-format line). Removed the top-of-file G10 build-discipline callout (it described chunked JSX assembly, which no longer applies; the DOCX is generated by a Python script, not iterative create_file writes). Older changelog entries below (v2.2-v2.3) describe the removed dashboard's history and are left as the historical record.
...
- **Aggregate stakeholder score rollup** -> `final_score_rollup.csv`
...
- Evaluation Schemas (inlined below) - Required sections and structures for all outputs
...
- *Deliverable format:* if file-creation and code execution are available, produce the rich artifacts this skill specifies (the branded `evaluation_report.docx` via `evaluation_report_generator.py`, plus the CSV pipeline artifacts). If they are not - e.g. running inside Word - produce the in-document equivalent: structured tables, headings, and summaries that live in the document. A missing renderer never means no deliverable.
...
- **Award Scenario Comparison** (when a split award is genuinely under consideration, e.g. 3+ eligible suppliers or the requesting stakeholder has raised a multi-award option): a static single-vs-split comparison, computed and written as a subsection rather than modeled live. Eligible suppliers are Award Tier Preferred or Competitive AND no Must Have gate fail; a gate-failed or Below Threshold supplier is named as excluded, with why, never silently omitted. Present 2-3 illustrative scenarios as a comparison table (Single Award to the rank-1 recommendation; an even split across eligible suppliers; and, where the evaluation team has proposed one, a named split) with columns Blended Grand Total, Blended Coverage %, Blended Year-1 Cost, Blended 3-Year TCV, and Vendor Count. Every blended figure is computed by calling the same `weighted_score()` kernel function (`numeric_kernel.py`) the official scoring uses, with each scenario's allocation percentages (fractions summing to 1.0) as the weight set and each eligible supplier's own official Grand Total, coverage, and commercial figures as the "scores" -- this is the same "consume, don't re-derive" discipline as the rest of this report, not a new scoring path. A supplier missing Phase 3 commercial figures is labeled NEEDS_INPUT for that scenario rather than assigned a fabricated cost. Close with 1-2 paragraphs of narrative trade-off: governance simplicity and per-unit pricing favor a single award; risk-hedging and best-of-breed-by-category favor a split, at the cost of coordinating multiple concurrent implementations.
