...
**The shipped `dashboard/assets/seed/category-data.js` is a demo/reference example, not live data, and must be fully replaced before building.**
- Its own leading comment says so: "the platform page's own demo data, captured per category from its Deep Analysis renderer." Every `$src` citation in it carries `"stub":true`, styled identically to a real citation (e.g. `"name":"ARIA S2P, PO Product pull","tier":1,"confidence":"Medium","asOf":"..."`) with no visible marker in the rendered dashboard distinguishing it from genuine, sourced data.
- **Author a complete replacement, never a partial edit or append.** Gather real figures for the actual category and commodity being analyzed (ARIA, SharePoint/M365 search, user upload, or web research per this skill's workflow and the resolution-tier chain in `panel_sources.json`), and write a fresh `category-data.js` in the same schema. Match its shape; do not reuse its content. A field with no resolved real source becomes `NEEDS_INPUT` or the shipped `stub`/`derived` provenance shape is left off entirely, never carried forward with `"stub":true` still attached to a value presented as real.
- `dashboard/build_dashboard_category.py` enforces this: it refuses (raises `StubDataError`) to build a non-demo dashboard if the seed still carries any `"stub":true` citation. This is a backstop, not a substitute for actually replacing the seed; a run that hits this error means the data-gathering step was skipped or incomplete, go back and do it.
...
- **Suite-wide guardrails note:** This skill follows the suite Execution Guardrails G1-G13 (tool-selection rules, mandatory gate checks, definition tracing, data-model-first for dashboards, pass-artifact enforcement, anti-collapse signal, pre-delivery self-tests). The canonical full text is `the "## INLINED: references/execution-guardrails.md" section inside /mnt/skills/user/lilly-brand-assets-1c344a/SKILL.md` (same mechanism this skill's own Operating Rule 9 already points at); a convenience copy also exists at `lilly-brand-assets-1c344a/references/execution-guardrails.md` for reference. This is a suite-wide note, not a per-skill version.
- **Changelog:** (newest first)
  - v4.5 (Jul 2026, Suite v10.7.0): Closed a data-integrity gap found during a package review: the shipped `dashboard/assets/seed/category-data.js` is the platform page's own demo data (its own leading comment says so), carrying `"stub":true` on all 170 of its `$src` citations, styled identically to a real ARIA citation with no visible marker anywhere in the rendered dashboard. Added an explicit SKILL.md warning (this section, above) instructing a full replacement of the seed with real gathered data before building, matching the clarity supplier-landscape-1c344a already had for its own seed ("a complete worked example: it IS the schema, match its shape"). Added a hard backstop in `dashboard/build_dashboard_category.py`: a new `StubDataError` refuses a non-demo build if any `"stub":true` citation survives in the seed, with an `--allow-stub` escape hatch reserved for maintainer/reference builds (never for a real run). Verified the two shipped reference HTML files (`category-strategy-dashboard.html`, `category-strategy-dashboard-DEMO.html`) still rebuild byte-identical under `--allow-stub`, so this is a new gate, not a behavior change to the shipped reference artifacts. New self-test (`--self-test`, 4/4) verifies the refusal fires for a non-demo build, is bypassed by `allow_stub=True`, and never fires for a demo build (where stub data is expected). No change to DEVELOP, MANAGE, or PREPARE mode behavior.
...
**Build it:** author the data object (a full, fresh replacement of the shipped seed with real
gathered data -- see "The shipped ... seed data is a demo/reference example" above), then run
`python dashboard/build_dashboard_category.py`. If the seed still carries any `stub:true`
citation, the builder refuses with `StubDataError` rather than shipping an unlabeled demo dashboard;
go back and finish the data-gathering step. There is no `create_file` assembly step and no
oversized-write risk, because you are not writing the artifact.
