...
# commercial-negotiation-prep-1c344a calls: escalate() (Year 2+ base fees and Escalation Impact), percentile_gate() (the per-family benchmark gate, and the Weighted Avg Position P[XX]-reportability gate in commercial_negotiation_generator.py), to_hourly() (rate normalization), weighted_score() (the volume-weighted Pricing Position Summary figure in commercial_negotiation_generator.py), and assert_reconciles() (every per-line rollup against its stated KPI/ZOPA total, including Total Annual Exposure and Total Leverage Value in commercial_negotiation_generator.py). The remaining functions are vendored for completeness (this is a verbatim copy of the shared kernel) but not called by this skill's own workflow.
"""
numeric_kernel.py
