...
# commercial-negotiation-prep-1c344a calls: escalate() (Year 2+ base fees and Escalation Impact), percentile_gate() (the per-family benchmark gate), to_hourly() (rate normalization), and assert_reconciles() (every per-line rollup against its stated KPI/ZOPA total). The remaining functions are vendored for completeness (this is a verbatim copy of the shared kernel) but not called by this skill's own workflow.
"""
numeric_kernel.py
