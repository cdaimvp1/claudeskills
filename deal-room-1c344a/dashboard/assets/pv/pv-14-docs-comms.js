...
function openDetails(){$('#drawer').innerHTML=`<div class="dh"><div><h3>Project details</h3><div class="dsub">P-1042 · Analytics SaaS, Visier AI</div></div><div class="dc" onclick="closeDrawer()">×</div></div><div class="db"><p class="narr">A new engagement to stand up an <b>AI-powered employee-analytics platform</b> from <b>Visier</b>, a vendor not previously under contract. Because it will <b>process employee PI</b> and involves <b>AI</b>, intake classified it <b>Orange</b> and fanned the work out to Legal, Cyber (ISS), the AI Registry (AIS) and WwTP risk in parallel, with contract authoring in LEAH and supplier enablement. Sourcing was skipped (sole source). It's a 3-year MSA + initial WO, ~$1.8M, currently held at the Reviews gate awaiting WwTP and the Cyber questionnaire.</p><div class="kv"><div class="k">Type</div><div class="v">New supplier engagement</div><div class="k">Supplier</div><div class="v">Visier (new) · HQ USA</div><div class="k">Contracts</div><div class="v">New MSA (full MPT) + initial Work Order</div><div class="k">Spend</div><div class="v">$1.8M TCO · 3 yrs</div><div class="k">CCI classification</div><div class="v">Orange, employee PI</div><div class="k">Standards</div><div class="v">SPS · ISS · AIS</div><div class="k">Owner</div><div class="v">Priya Shah</div><div class="k">Rep</div><div class="v">Marc Lane</div><div class="k">Started</div><div class="v">2026-06-23</div><div class="k">Target PO</div><div class="v">2026-07-31</div></div></div>`;$('#scrim').classList.add('on');$('#drawer').classList.add('on');}
...
function renderPinned(){const v=VIEWS[view],f=v.nm.split(' ')[0];
 const nudges=[
  {id:'pv-draft-chase',cls:'assist',pz:'✦',msg:`<b>${llm}</b> can draft a status-chase email to Visier`,act:`<button class="btn btn-ghost btn-sm" onclick="aiDraft()">Draft</button>`},
...
function aiDraft(){CHAT.push({who:llm+' assistant',av:'✦',cls:'ai',model:llm,tm:nowStamp(),tx:'Here\'s a draft to Visier asking for status on the security questionnaire and an ETA, review and send when you\'re happy:',draft:{to:'sam@visier.com',subj:'Status check, security questionnaire (ref P-1042)',body:'Hi Sam,\n\nFollowing up on the information-security questionnaire we shared for the analytics engagement. Could you share the current status and an estimated date you\'ll return it? It\'s the last item the third-party risk review is waiting on before we can proceed.\n\nHappy to jump on a quick call if easier.\n\nThanks,\nPriya'}});renderChat($('#csearch').value);}
// #E2: a status-UPDATE variant (to internal stakeholders) alongside the supplier chase
function draftStatusUpdate(){CHAT.push({who:llm+' assistant',av:'✦',cls:'ai',model:llm,tm:nowStamp(),tx:'Here is a status update you can send to the project stakeholders. It summarizes where P-1042 stands, what it is waiting on, and the projected date. Review and send when you are happy:',draft:{to:'priya.shah@lilly.com; it-procurement-leads@lilly.com',subj:'Status · P-1042 Visier (Orange) · awaiting WwTP + Cyber',body:'Team,\n\nQuick status on the Visier engagement (P-1042 · $1.8M · 3-yr MSA):\n\n- Stage: parallel reviews. Legal (MSA) on track; AI Registry cleared.\n- Waiting on: Visier to return the WwTP risk questionnaire (overdue) and the Cyber/ISS questionnaire (at risk).\n- Contract: on v3; held the 1x liability cap with a PI carve-out.\n- Projected: the reviews gate clears ~7/1 if the questionnaires land this week; target PO 7/31.\n\nI will flag if WwTP slips further.\n\nThanks,\nMarc'}});renderChat(($('#csearch')||{}).value||'');}
...
function summarizeTurn(){var sum='Here is Visier\'s last turn ('+(INBOUND.from?INBOUND.from.name:'Visier')+', '+(INBOUND.attachment||'redline')+') summarized - what changed and what they are asking:<br><br>'+
 ...
 '<b>Countered (5):</b><br>1. Liability cap - Visier wants 2x fees (we hold 1x + PI carve-out).<br>2. Sub-processors - Visier wants notice-only (we want prior-approval).<br>3. Renewal - then-current list (we want a CPI / 3% cap).<br>4. IP - Visier retains models trained on Lilly data (we want Lilly-data outputs).<br>5. Indemnity - Visier caps PI-breach indemnity (we want it uncapped).<br><br>'+
