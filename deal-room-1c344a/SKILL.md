...
- **Suite-wide guardrails note:** Execution guardrails G1-G11 apply suite-wide (tool-selection rules, mandatory gate checks, definition tracing, data-model-first for dashboards, research minimums, pre-delivery self-tests, and kernel-backed computation for the subset of skills, including this one, that vendor a numeric kernel). See `the "## INLINED: references/execution-guardrails.md" section inside /mnt/skills/user/lilly-brand-assets-1c344a/SKILL.md`.
