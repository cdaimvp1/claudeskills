...
| meeting-prep-brief | rfp-case-manager Schedule workflow (after drafting an invite) | process-navigator (policy/threshold questions), voice-profile (opt-in follow-on drafts), workflow-map (opt-in workflow diagram) |
...
| timeline-builder | process-navigator (prerequisite questions feeding factor extraction) | voice-profile (drafts a status email from the estimate) |
...
| workflow-map | rfp-case-manager (Initialize and Refresh workflows) | process-navigator (which phases/reviews apply), timeline-builder (duration labels), rfp-case-manager (stakeholder roster, read-only) |
