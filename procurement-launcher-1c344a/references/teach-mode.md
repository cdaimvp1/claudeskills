...
- Personal Command Center pipeline (five skills): voice-profile, process-navigator, timeline-builder, workflow-map, and meeting-prep-brief. (theos-field-guide, the work-graph command center, is held back and not part of the shipped package; workflow-map's optional `issue_id` parameter and a few other skills' opportunistic Field Guide integrations degrade gracefully to their non-Field-Guide defaults when it is absent.)
