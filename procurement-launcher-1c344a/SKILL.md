...
description: >
  ...
  split-panel menu (seven pipelines as collapsible sections, a hover-driven detail panel,
...
- It is a **light router**. It presents a split-panel master-detail menu (seven pipeline sections collapsible on the left, a hover-driven detail panel on the right, click any skill row to launch it) and hands off.
...
2. **Names the full ordered path up front**, drawn ONLY from
   ...
   procurement-options-analysis -> supplier-landscape -> rfp-engine ->
   rfp-response-analysis -> evaluation-engine -> decision-deck. We'll start at step 1;
   after each one I'll point you to the next."
...
- **Net-new competitive buy, path unclear** ("I need to buy/select a tool for X"):
  procurement-options-analysis (pick the path) -> supplier-landscape (shortlist) ->
  rfp-engine (RFx package + requirements grid) -> rfp-response-analysis (analyze
  responses) -> evaluation-engine (official score + award) -> decision-deck (exec deck).
- **Buy from / expand a known incumbent** (options-analysis "expand incumbent" branch):
  procurement-options-analysis -> commercial-negotiation-prep (or lilly-contract-review
  if a document is already in hand) -> pro-forma-builder / should-cost-builder (cost
  case) -> deal-room (run the live negotiation) -> negotiation-playbook-learning (record
  the outcome at close).
...
- **2 - Sourcing / RFx** - options analysis (which path), supplier search, single-supplier deep dive, sole-source challenge, RFx package, active-case management, response analysis, scoring and award
...
- **5 - Decision Deck** - build an executive deck from any inputs
- **6 - Executive Summary (ATC / ATS)** - turn a contract or WO into an approval summary
- **7 - Personal Command Center** - voice profile, Theo's Field Guide, process navigator, timeline builder, workflow map, meeting prep brief, procurement help desk (pending)
...
- **2 - Sourcing / RFx:** Procurement Options Analysis ("options analysis") / Supplier Search ("find vendors") / Single-Supplier Deep Dive ("profile this supplier") / Sole-Source Challenge ("challenge this sole source") / Build an RFx Package ("create an RFP") / Manage an Active RFx ("set up a case for this RFx") / Analyze Supplier Responses ("analyze the supplier responses") / Score Proposals and Award ("score the proposals")
...
1. **All seven pipelines are visible at once as collapsible section headers** in the left column (closed by default; only one open at a time enforced by JS that collapses siblings on open). Tapping a header expands its skill rows inline using native HTML `<details>`/`<summary>` with a small JS layer that enforces the single-open rule and refreshes the right panel.
...
This table is THEO's single registry: the seven `Pipeline` labels below are exactly the seven menu sections (widget + Markdown fallback + Teach-mode lists all derive from these). The suite is **29 routable skills organized into seven pipelines**: Contracts & Negotiations (12 rows / 8 distinct skills, lilly-contract-review and negotiation-simulator each fire several artifact-specific rows), Sourcing / RFx (8), Category Strategy (4 rows / 2 skills: category-strategy DEVELOP/MANAGE plus market-rate-benchmarking's external and rationalization modes; its third, internal mode is not surfaced on the menu, per the v1.8 curation pass), Cost & Commercial (3), Decision Deck (1), Executive Summary (1), Personal Command Center (6). Skill count: lilly-contract-review, legal-negotiation-prep, commercial-negotiation-prep, comment-cleanup, negotiation-playbook-learning, negotiation-simulator, scope-sow-architect, deal-room (8); supplier-landscape, supplier-deep-dive, rfp-engine, rfp-case-manager, rfp-response-analysis, evaluation-engine, procurement-options-analysis, sole-source-challenge (8); category-strategy, market-rate-benchmarking (2); pro-forma-builder, should-cost-builder, invoice-rate-card-auditor (3); decision-deck (1); executive-summary-package (1); voice-profile, theos-field-guide, process-navigator, timeline-builder, workflow-map, meeting-prep-brief (6). Total = 29 built routable skills. **Count reconciliation:** a 30th entry, procurement-help-desk (last row below), is a PENDING end-user/stakeholder front door whose content build is network-gated (Stage 7); it is listed so it is not stranded but is NOT counted in the 29 built and is not on the interactive widget yet. The lilly-brand-assets foundation and procurement-launcher (THEO) itself are infrastructure, not menu destinations (THEO surfaces lilly-brand-assets only via the troubleshooting/manual pointer, and THEO is the menu, not a row in it); counting them, the full packaged suite is 31 skills plus the pending help desk. The menu routes to the 29 built specialist skills.
...
| Sourcing / RFx | procurement-options-analysis | Decide WHICH PATH for one request (buy externally / build / expand incumbent / reuse / consolidate / defer / run RFI / run RFP / direct-negotiate-with-validation / pilot-first), scored on cost, time, feasibility, switching burden, risk, optionality; the upstream triage step that hands the chosen path to the skill that executes it | "options analysis" / "should we buy or build" / "which path should we take" | nothing - a described need is enough | current-state context, deal-value magnitude, urgency, a prior supplier-landscape or cost anchor |
...
| Decision Deck | decision-deck | Build an executive deck from any inputs | "build a deck for [audience]" | nothing - any inputs or a verbal description | dashboards, evaluations, contracts, pro-forma models, spend data |
