...
- **Author:** Marc Lane, Associate Director, Global IT Procurement
- **Requires:** lilly-brand-assets v10.0+ (shared foundation); lilly-contract-review v3.5+; negotiation-simulator v2.3+ (for phrase-carried mode detection); rfp-engine v2.3+; evaluation-engine v2.2+; rfp-case-manager v2.2+; process-navigator v2.0+ (now covers stakeholder help-desk questions, see below)
- **Changelog:**
  - v2.13 (July 31, 2026): **procurement-help-desk folded into process-navigator, no longer a pending standalone skill.** Marc decided the fold-in-as-a-mode path that process-navigator's own SKILL.md had left open since Stage 7. Removed the PENDING procurement-help-desk row from the routing table and the count-reconciliation paragraph (it never counted toward the 26 built or 28 packaged, so those totals are unchanged; only the "plus the pending help desk" caveat is gone). Updated process-navigator's own row to state it now covers both procurement-rep and stakeholder audiences, with stakeholder trigger phrases added to Say-this. Also updated `references/routing-and-chains.md` and `references/teach-mode.md` to remove the pending-entry notes there.
...
- **6 - Personal Command Center** - voice profile, process navigator, timeline builder, workflow map, meeting prep brief, procurement help desk (pending)
...
- **6 - Personal Command Center:** Voice Profile ("build my voice profile") / Process Navigator ("how do I buy this") / Timeline Builder ("estimate the timeline for this request") / Workflow Map ("build a workflow map for this request") / Meeting Prep Brief ("prep me for this meeting") / Procurement Help Desk (PENDING; stakeholder front door, "how do I onboard a supplier" / "how do I check an invoice status")
...
This table is THEO's single registry: the six `Pipeline` labels below are exactly the six menu sections (widget + Markdown fallback + Teach-mode lists all derive from these). The suite is **26 routable skills organized into six pipelines**: Contracts & Negotiations (13 rows / 8 distinct skills, lilly-contract-review and negotiation-simulator each fire several artifact-specific rows), Sourcing / RFx (7), Category Strategy (4 rows / 2 skills: category-strategy DEVELOP/MANAGE plus market-rate-benchmarking's external and rationalization modes; its third, internal mode is not surfaced on the menu, per the v1.8 curation pass), Cost & Commercial (3), Executive Summary (1), Personal Command Center (5). Skill count: lilly-contract-review, legal-negotiation-prep, commercial-negotiation-prep, comment-cleanup, negotiation-playbook-learning, negotiation-simulator, scope-sow-architect, deal-room (8); supplier-landscape, supplier-deep-dive, rfp-engine, rfp-case-manager, rfp-response-analysis, evaluation-engine, sole-source-challenge (7); category-strategy, market-rate-benchmarking (2); pro-forma-builder, should-cost-builder, invoice-rate-card-auditor (3); executive-summary-package (1); voice-profile, process-navigator, timeline-builder, workflow-map, meeting-prep-brief (5). Total = 26 built routable skills. **Count reconciliation:** procurement-help-desk, previously listed here as a PENDING stakeholder front door, was folded into process-navigator as a second mode on 2026-07-31 rather than shipped as its own skill; process-navigator's row above now covers both the procurement-rep and stakeholder audiences, and there is no separate pending entry to track. The lilly-brand-assets foundation and procurement-launcher (THEO) itself are infrastructure, not menu destinations (THEO surfaces lilly-brand-assets only via the troubleshooting/manual pointer, and THEO is the menu, not a row in it); counting them, the full packaged suite is 28 skills. The menu routes to the 26 built specialist skills.
...
| Contracts & Negotiations | comment-cleanup | **Finalize a document for signature (HIGHER-STAKES: accepts agreed tracked changes into the body text)** | "finalize this for signature" / "make this execution-ready" / "produce a clean copy" | the redlined DOCX with agreed changes | confirmation that the changes really are agreed |
...
| Personal Command Center | process-navigator | Procurement process / policy / threshold / system-requirement Q&A for procurement reps, AND stakeholder how-to/where-to-go/who-to-contact/status Q&A for non-procurement staff (two modes, one skill since 2026-07-31); live-reads Lilly SharePoint policy sources via M365 | "how do I buy [X]" / "do I need TPRM / SAE / AIR" / "what's the FRAP threshold" / "can I use PO T&Cs vs full MSA" / "how do I onboard a supplier" / "where do I start to buy this" / "who do I contact for [need]" / "what's the status of my invoice" | nothing - just ask the question | category / commodity context; dollar value; use-case context (rep mode); PO/invoice number or supplier name (stakeholder status-check) |
