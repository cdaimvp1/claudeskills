...
- Summary of the guardrails (G1-G13):
  ...
  - **G11 (Kernel-Backed Computation):** This skill vendors a decision kernel (`tier_kernel.py`), an LLM/Python HYBRID kernel as of v2.3. The Phase 2 tier assignment (Red Line / Hold Firm / Strategic Trade / Easy Concede) MUST be produced by calling `assign_tier()` in that kernel, never by model prose reasoning against the tier table, for every term. For a term the kernel confirms maps to a playbook position or conflicts with the governing MSA, the kernel's tier is BINDING (the LLM cannot override it, and can never soften a Hard Stop RED LINE). For a commercial term the kernel confirms is outside playbook and MSA scope, the kernel returns `NEEDS_LLM` and the LLM must assign the tier itself with a grounded rationale, a confidence score, and a citation, never a bare guess. See "Kernel Wiring (G11, HARD RULE)" below for the exact function, inputs, and call site.
...
- **Requires:** lilly-brand-assets v10.0+ (shared foundation); optionally consumes lilly-contract-review-1c344a (playbook/SME/pharma references) and negotiation-playbook-learning-1c344a (history) when installed. Vendors its own decision kernel `tier_kernel.py` (in this skill's own directory), reworked in v2.3 into an LLM/Python HYBRID kernel for the Phase 2 tier assignment (G11): Python is BINDING for playbook and governing-MSA-conflict terms; the LLM assigns the tier, grounded, only for commercial terms the kernel confirms are outside playbook and MSA scope.
- **Changelog:**
  - v2.3 (July 2026): Reworked `tier_kernel.py` from a pure deterministic kernel into an LLM/Python HYBRID kernel. Added two explicit inputs, `is_playbook_position` (playbook-match) and `msa_conflict` (MSA-conflict), and a new Applicability Gate: a Hard Stop/regulatory term is still RED LINE and now explicitly LOCKED (the LLM can never soften it); a playbook-matched or MSA-conflicting term runs the original Tier Assignment Decision Tree (Tests 1-4, Overrides A/B/C, unchanged) and its result is now BINDING (the LLM cannot override it), with a new MSA-Conflict Floor guaranteeing HOLD FIRM minimum for any MSA-conflicting term; a commercial term that is neither in the playbook nor MSA-conflicting now returns `NEEDS_LLM` instead of a kernel-assigned tier, requiring the LLM to assign that tier itself with a grounded rationale, a confidence score, and a citation, never a bare guess. Every result now carries a `source` field (`playbook` | `msa` | `llm`) for auditability. Updated the Phase 2 workflow text and "Kernel Wiring (G11, HARD RULE)" section below to match. See tier_kernel.py's module docstring, "v2.0 HYBRID KERNEL ADDITION," for the full design rationale.
...
- **Suite-wide guardrails note:** Execution Guardrails G1-G13 (tool-selection rules, mandatory gate checks, definition tracing, data-model-first for dashboards, pass-artifact enforcement, anti-collapse signals, pre-delivery self-tests, and kernel-backed computation for the tier assignment this skill vendors a kernel for) apply suite-wide. See the GLOBAL OPERATING RULES section above.
...
**Computation requirement (HARD RULE): do not hand-classify the tier.** Call `assign_tier(term_attrs)` in the vendored `tier_kernel.py` (in this skill's own directory) for EVERY term, never by model prose reasoning against the table above. As of v2.3 this is an LLM/Python HYBRID kernel: which of the four tiers a term lands in is deterministic and BINDING for legal terms and terms that may conflict with the governing MSA; for a commercial term the kernel confirms is outside playbook and MSA scope, the kernel abstains and the LLM assigns the tier itself, grounded.

Build `term_attrs` from what Phase 1 already gathered: Hard Stop/regulatory status, **and, new in v2.3, the two Applicability Gate inputs** - `is_playbook_position` (does this term map to a playbook position: a standard position, required protection, or fallback the playbook already covers for this contract type?) and `msa_conflict` (does this term conflict with, or fall below, a provision of the governing MSA?) - plus per-position compliance leverage, historical acceptance rate, fallback availability, financial exposure, supplier precedent count, and portfolio precedent risk. Pass all of it to `assign_tier()`. Do not guess `is_playbook_position` or `msa_conflict`: they come from the Phase 1A governing-document read and the Source A playbook read, the same passes that already populate the Position Map & MSA Coverage panel's "MSA Already Covers" list and the playbook-position set. If either is genuinely undetermined, `assign_tier()` returns `REVIEW` naming it; gather it and re-run rather than assuming.

`assign_tier()` returns one of four outcomes:
- `TIER_RED_LINE` / `TIER_HOLD_FIRM` / `TIER_STRATEGIC_TRADE` / `TIER_EASY_CONCEDE` with `source="playbook"` or `source="msa"` and `binding=True`: the term is a Hard Stop/regulatory requirement, maps to a playbook position, or conflicts with the governing MSA. **This tier is BINDING (HARD RULE): do not override it with model judgment, and never soften a `source="playbook"` RED LINE result under any circumstance**, no matter what commercial context or supplier pressure narrative would otherwise argue for a softer read.
- `NEEDS_LLM` with `source="llm"`, `tier_number=None`: the term is commercial, is NOT a playbook position, and does NOT conflict with the governing MSA - the kernel has no deterministic basis and abstains rather than fabricate a tier. **You (the LLM) must assign the tier directly** (one of the same four tiers), and the assignment MUST include a grounded rationale (why this tier, referencing the term's actual risk/value, not a generic label), a confidence score (High/Medium/Low, per the suite's ground+confidence+abstain rule), and a citation (what the rationale is grounded in: market/category norm, prior negotiation, the specific commercial fact pattern). A bare tier with no rationale, confidence, or citation is not an acceptable substitute for a `NEEDS_LLM` assignment and must not be presented as final.
- `REVIEW`, `source=None`: a required attribute for the next gate or test is missing (this includes `is_playbook_position` and/or `msa_conflict` when neither is known yet). Surface it as NEEDS_INPUT (per Global Operating Rule 8) naming exactly which attribute `assign_tier()` reported missing, gather that input, and re-run.

See "Kernel Wiring (G11, HARD RULE)" immediately below for the full call-site table and the binding/NEEDS_LLM handling in detail.

This tier classification (kernel-binding or LLM-assigned-per-NEEDS_LLM) is the source for two renders later in the document: the full tactical detail in Sections 01-04 and the Quick Reference Card (Section 08), and the compact tier-by-position count and headline-item preview in the Position Map & MSA Coverage panel (see Phase 5 and the inlined design spec below), which sits on page 1 right after the Executive Summary. Classify once; both renders read from the same tier assignments, and any LLM-assigned (NEEDS_LLM) tier carries its rationale, confidence, and citation into both renders so a reader can tell a kernel-binding tier from an LLM-grounded one.
...
- [ ] Every term relevant to this contract type run through `assign_tier()` in `tier_kernel.py`, not classified by prose (per G11)
- [ ] Every term returning a binding tier (`source="playbook"` or `source="msa"`) recorded as-is: not overridden, and no `source="playbook"` RED LINE softened, regardless of commercial argument
- [ ] Every term returning `NEEDS_LLM` assigned a tier directly by the LLM, with a grounded rationale, a confidence score, and a citation recorded alongside it - never a bare tier
- [ ] Every term returning `REVIEW` logged as NEEDS_INPUT with the missing attribute(s) named (including `is_playbook_position` / `msa_conflict` when either was undetermined), not force-assigned a tier
...
This skill vendors `tier_kernel.py` in its own directory: an LLM/Python HYBRID kernel as of v2.3. The following decision MUST be produced by calling the kernel for every term, never by model prose:

| Computation | Kernel function | Where it appears |
|---|---|---|
| Tier assignment for every term (Red Line / Hold Firm / Strategic Trade / Easy Concede), per the Tier Assignment Decision Tree for playbook/MSA terms, or NEEDS_LLM for commercial terms outside playbook/MSA scope | `assign_tier()` | Phase 2 (Position Analysis & Prioritization); Section 01-04 position cards; Position Map & MSA Coverage panel quadrant; Quick Reference Card (Section 08) |

`assign_tier()` takes a `TermAttrs` record built from the structured attributes this skill already classifies per term: Hard Stop/regulatory status; **`is_playbook_position`** (the playbook-match input - True when the term maps to a playbook position); **`msa_conflict`** (the MSA-conflict input - True when the term conflicts with the governing MSA); per-position compliance leverage; historical acceptance rate; acceptable-fallback availability; financial exposure as a fraction of contract value; count of prior negotiations where the supplier accepted Lilly's standard; and whether conceding creates portfolio precedent risk.

It returns a `TierResult` carrying `decision`, `tier_number`, `trace`, `missing_inputs`, and two fields new in v2.3:
- **`source`** (`"playbook"` | `"msa"` | `"llm"` | `None`): which path decided the result, for auditability. Render this alongside every position card and quadrant entry so a reviewer can immediately tell whether a tier is kernel-binding (playbook/msa) or LLM-assigned (llm).
- **`binding`** (`True` / `False`): `True` only when `source` is `"playbook"` or `"msa"`. A `binding=True` result is a HARD RULE: the LLM must not override it, must not "round down" a Hard Stop RED LINE to something more negotiable, and must not relabel an MSA-conflict-floored HOLD FIRM as tradeable.

**Applicability Gate logic (how the kernel decides playbook/msa vs. llm):**
- Hard Stop or regulatory requirement -> `TIER_RED_LINE`, `source="playbook"`, `binding=True`. Locked: never softened by the LLM, under any circumstance.
- Not a Hard Stop, but `is_playbook_position=True` or `msa_conflict=True` -> the original Tier Assignment Decision Tree runs (Tests 2-4, Overrides A/B/C, unchanged from prior versions) and, when `msa_conflict=True`, a new MSA-Conflict Floor guarantees `TIER_HOLD_FIRM` minimum regardless of what the tree computed. Result is `source="playbook"` (if `is_playbook_position`) or `source="msa"` (if only `msa_conflict`), always `binding=True`. Note: `is_playbook_position=True` alone does NOT force Hold Firm - a genuinely low-priority playbook position (low acceptance rate, safe fallback) can still legitimately land at STRATEGIC TRADE or EASY CONCEDE, exactly as the original tree intends (see the Invoice Frequency example in the Position Map).
- Not a Hard Stop, `is_playbook_position=False`, and `msa_conflict=False` -> `NEEDS_LLM`, `source="llm"`, `binding=False`. This is the commercial-term-not-in-playbook case: the kernel confirms there is no playbook or MSA basis and hands the assignment to the LLM, which must ground it (rationale + confidence + citation), never guess it.
- Either gate input (`is_playbook_position`, `msa_conflict`) unknown -> `REVIEW`, naming the missing input(s). Do not assume a term is or is not in the playbook, or does or does not conflict with the MSA; determine it from the Phase 1A governing-document read and the Source A playbook read, then re-run.

A `REVIEW` result is not a fabricated tier: it names the exact missing attribute(s). Treat it as NEEDS_INPUT for that position (per Global Operating Rule 8), gather the missing attribute, and re-run `assign_tier()` before including that position in Sections 01-04 or the Quick Reference Card.

A `NEEDS_LLM` result is not a fabricated tier either: it is the kernel's explicit hand-off. Assign the tier yourself, grounded with a rationale, a confidence score (High/Medium/Low), and a citation, and carry all three into the position card alongside the assigned tier and `source="llm"` label, consistent with the suite's deterministic-primary + ground/confidence/abstain rule and the Rule 3 confidence-signaling requirement above. A tier with no rationale, confidence, or citation is not a valid `NEEDS_LLM` resolution.

If `tier_kernel.py` is missing or errors on the given input, STOP and report the failure; do not fall back to classifying the position by prose reasoning against the tier table (per the suite's G11 rule). A tier assignment that did not come from calling `assign_tier()` (for the binding path) or from a grounded LLM assignment following a `NEEDS_LLM` result (for the commercial-not-in-playbook path) is invalid and must not be presented as final.
...
**v2.3 hybrid-kernel note:** the tree below is unchanged, and still governs every term the kernel confirms is a Hard Stop/regulatory requirement, a playbook position (`is_playbook_position=True`), or MSA-conflicting (`msa_conflict=True`). As of v2.3, `assign_tier()` first runs an Applicability Gate on those two new inputs: if neither is true (a commercial term, not in the playbook, not MSA-conflicting), the kernel does NOT apply this tree at all - it returns `NEEDS_LLM` and the LLM must assign the tier itself, grounded (rationale + confidence + citation). When `msa_conflict=True`, a new MSA-Conflict Floor runs after the Overrides below and raises the result to Tier 2 (HOLD FIRM) minimum if the tree/overrides computed a weaker tier. Every result from this tree is BINDING (the LLM cannot override it); a Tier 1 (RED LINE) result from Test 1 can never be softened. See "Kernel Wiring (G11, HARD RULE)" above for the full Applicability Gate logic.
...
- **(NEW, v2.3) Any position where the term conflicts with the governing MSA (`msa_conflict=True`): minimum Tier 2 (HOLD FIRM), applied after the three overrides above (the MSA-Conflict Floor). This override is new to v2.3 and is not part of the original tree text; it is additive, not a re-derivation of the three overrides above.**
