...
- **Version:** 0.3 (LIVE-VALIDATED - the network-gated steps have been run; see changelog)
- **Suite:** v10.7.0
...
- **Changelog:**
  - v0.3 (July 31, 2026): Confirmed by Marc that "Buy@Lilly Support" is the BuyLilly site itself (`collab.lilly.com/sites/Buylilly`), not a separate destination -- corrected `references/buylilly-where-to-start-a-buy.md` to say so directly. Added Knowledge Source 6, the Source to Pay Home page (confirmed by Marc as a real, distinct site), the richest systems-map source found so far: real Aravo buyer-login URL, a SAP inquiry tool likely underlying "Status Hub," a likely-LEAH ContractPod URL, the real Buy@Lilly ServiceNow support-case link, and confirmation that WwTP's own front door is on now.lilly.com (same unreachable-by-connector category as ProtectLilly, not previously known). Updated `procurement-operating-model.md`'s systems map and `buylilly-invoice-status.md` / `buylilly-supplier-onboarding.md` with these real URLs.
...
6. **Source to Pay Home page** -- the real systems-map front door, confirmed by Marc 2026-07-31
   ...
   A distinct site from the Playbook (different SharePoint path). "SOURCE TO PAY | Supports for Suppliers" hub with quick-link tiles to BUYING / SUPPLIER LIFECYCLE / INVOICING / PAYMENTS / FRAP POLICY / CONTACT PROCUREMENT, and direct links to the real transactional systems (Ariba realm URLs, the Aravo buyer login, a SAP inquiry tool labeled "Access SAP Supplier, Purchase Order, Invoice or Payment information," a likely-LEAH ContractPod URL, the WwTP front door on now.lilly.com, and the real Buy@Lilly ServiceNow support-case link). Fetch this for "which system do I actually use" and "where's the real link" questions; the direct system URLs it surfaces are login-gated (name and link them, never fetch their content). See `references/procurement-operating-model.md` for the full extracted systems map.
