...
**What this scale does NOT govern** (these are deliberately separate metrics, not evaluation scores, and must NOT be rescaled to 0.0-5.0): supplier-landscape's discovery-stage capability-fit score (a 0-10 shortlisting tool that is never handed off downstream as a number; its hand-off carries only categorical Strong/Moderate/Weak), the 0-100 contract Protection Score in `risk-scoring.md`, negotiation difficulty (0-100), data-quality (0-100), and **theos-field-guide's Report Card GPA (a 4.0-point academic scale)**.

The Report Card GPA was added to this list on 2026-07-29. It had been in use without appearing here, which made this list read as exhaustive when it was not. Two properties matter and neither is obvious from the number: it is **self-scored by the user, not derived** from the graded categories rendered beneath it, and the categories themselves sit on heterogeneous scales (8/10, 7/10, 2.65M/4M) so no average of their values would be meaningful. It is never handed off to another skill. Per the legend rule below, the view prints "self-scored, 4.0 scale" beside the numeral.
