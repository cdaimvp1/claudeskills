...
- Generate executive presentations (decision-deck does that)
...
## Multi-Pass Generation Guidance

For large evaluations (4+ suppliers, 400+ requirements), the analysis_summary.docx will be 30-40 pages and CANNOT be generated in a single pass without sections regressing. Use multiple sequential passes:

1. **Pass 1:** Generate title page (per the LOCKED Title Page Structure), table of contents, executive summary, and the first 2 supplier sections (e.g., sections 1-3). Save the document.
2. **Pass 2:** Open the saved document and append the remaining supplier sections (e.g., sections 4-5), then the gated Section 5, **Bid Leveling & Normalization** (comparison basis, scope-compliance map, assumption and exclusion register, normalized pricing table, missing-cost placeholders, one-time vs recurring split, reported vs normalized TCO, and questions before final evaluation, across every supplier). Save.
3. **Pass 3:** Confirm the GATE CHECK: Bid Leveling Complete passes, then open the saved document and append all cross-cutting sections (6-13). Save. Do not begin Pass 3 until Bid Leveling is complete: the cross-vendor comparison, weighted scoring, pricing analysis, and Final Recommendation all read from the leveled, not the raw reported, figures.

Each pass opens the previously saved document and appends new content. This prevents the common failure mode where investing depth in cross-cutting sections causes supplier sections to be compressed, or vice versa.

**CRITICAL: Per-supplier sections (2-N) require the SAME depth and rigor as cross-cutting sections (6-13).** The supplier response analysis is the core analytical content of this report -- it is where the reader learns what each supplier actually proposed. A supplier submitting 140 pages of material deserves 4-6 pages of analysis with 7-8 subsections, not a compressed paragraph. If the generation environment cannot produce full-depth supplier sections AND full-depth cross-cutting sections in the same pass, split them across passes. Every section must maintain full depth regardless of which pass generates it.
...
|----------|------------|------------|
| rfp-engine (requirements matrix), rfp-case-manager (case file, submissions) | rfp-response-analysis | evaluation-engine (formal scoring), decision-deck (executive narrative) |
