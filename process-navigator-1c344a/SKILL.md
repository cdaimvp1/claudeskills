...
6. **Source to Pay Home page** -- the real systems-map front door, confirmed by Marc 2026-07-31
   ...
   A distinct site from the Playbook. Links directly to the real transactional systems (Ariba realm URLs, the Aravo buyer login, a SAP inquiry tool, a likely-LEAH ContractPod URL, the WwTP front door on now.lilly.com, and the Buy@Lilly ServiceNow support-case link). Useful for "which system do I actually use" and "where's the real link" questions; these system URLs are login-gated (name and link, never fetch content). Full extracted map in `procurement-help-desk-1c344a/references/procurement-operating-model.md` (shared across both skills).
