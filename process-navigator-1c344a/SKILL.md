...
4. **Global ProtectLilly** (intranet, not SharePoint)
   ...
   NOTE: this source lives on the now.lilly.com intranet, NOT on collab.lilly.com SharePoint. The M365 connector indexes SharePoint / OneDrive / Outlook / Teams, so it may NOT be able to read this page. Treat ProtectLilly as the source most likely to fail retrieval, and follow the per-source retrieval-failure handling in Step 3 when it does.
...
- **Global ProtectLilly (now.lilly.com intranet) failed:** this is expected, because the M365 connector indexes SharePoint / OneDrive / Outlook / Teams and may not reach now.lilly.com. Do NOT present TPRM / SAE / AIR / Privacy answers as Lilly-verified when ProtectLilly did not load. Instead: (1) say plainly "I could not read Global ProtectLilly from here (it is on the now.lilly.com intranet, which this connector may not index)"; (2) give the user the direct ProtectLilly link to open themselves; (3) answer from general third-party-risk principles labeled "not Lilly-verified" at Low confidence; and (4) point to the named SME route per Hard Rule 5.
