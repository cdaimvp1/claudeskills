...
# SMOKE-TEST FIX 2026-07-29 (G9). The body below ran at MODULE level, so merely
# IMPORTING this file executed file I/O against DEAL, an absolute path that no longer
# exists on any machine, and the import raised FileNotFoundError. That made this a
# Desktop failure inside an installable skill rather than the inert reference file it
# is documented to be.
#
# _deal_build/SOURCE-OF-TRUTH.md:50 records the decision: 'Left in place, headed as
# superseded, not deleted.' That decision is honoured. Nothing was removed; the
# execution is simply moved behind a main guard so the file is importable and inert
# unless deliberately run, which is what 'kept for reference' already meant.
