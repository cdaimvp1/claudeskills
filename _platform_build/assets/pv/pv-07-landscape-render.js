...
/* ===== SL.2 supplier-landscape LIVE read (ADDITIVE, reflect-only) =====
   ...
   dynamic value is escaped (escD); positive accent Bold Blue #0F3A85. */
...
function ovLandCardHTML(refl){
 ...
 return '<div style="border:1px solid #C6D7EF;border-left:3px solid #0F3A85;border-radius:10px;padding:9px 12px;margin:9px 0 0">'+
  ...
  '<div style="font-size:10.5px;color:var(--mut2,#8A827C);font-style:italic;margin-top:6px">Reflect-only, from the platform landscape engine over this project\'s own candidate set. Advisory: no vendor is selected or contacted.</div></div>';
}
...
/* ===========================================================================
   ...
   action sit on top. Reflect-only; positive accent Bold Blue #0F3A85 (never
...
const PVSLE=(function(){
  ...
  return {analyzeLandscape,reflect:function(input){return {landscape:analyzeLandscape(input)};}};
})();
...
var PVSEG_COLOR={leader:'#0F3A85',challenger:'#5C84AA',niche:'#9DB4D0',caution:'#C9A227',disqualified:'#B0A9A2'};
...
var PVQUAD_SHADE={leader:'rgba(18,60,130,.08)',challenger:'rgba(47,110,107,.11)',niche:'rgba(92,43,80,.10)',caution:'rgba(201,162,39,.13)'};
var PVQUAD_SOLID={leader:'#123C82',challenger:'#2F6E6B',niche:'#5C2B50',caution:'#B58A16'};
...
function pvFitBg(score){var a=0.10+(Math.max(0,Math.min(5,score))/5)*0.70;return 'background:rgba(15,58,133,'+pvRound(a,3)+');color:'+(score>=3.5?'#fff':'var(--ink)')+';';}
...
function pvBandBg(s){var b=pvBandOf(s);return b==='strong'?'background:#0F3A85;color:#fff':b==='adequate'?'background:var(--tint-e7a33e,#E7A33E);color:#3A2600':'background:#E1251B;color:#fff';}
/* lighter band tints for sub-requirement + field-average cells (hierarchy) */
function pvBandBgSub(s){var b=pvBandOf(s);return b==='strong'?'background:rgba(15,58,133,.16);color:var(--blue-d)':b==='adequate'?'background:rgba(231,163,62,.24);color:var(--amber-d)':'background:rgba(225,37,27,.14);color:var(--red-d)';}
...
function pvSrcTag(kind){
 ...
 if(kind==='int')return '<span title="Internal, from Lilly own records: spend, contracts, RFP pricing and relationship history." style="'+b+'background:rgba(15,58,133,.12);color:var(--navy);border:1px solid rgba(15,58,133,.30)">internal</span>';
 if(kind==='ext')return '<span title="External enrichment, firmographics, financials and analyst coverage from credible public sources; reflect-only, not validated." style="'+b+'background:var(--tint-eef0f3,#EEF0F3);color:#5B6472;border:1px solid rgba(0,0,0,.15)">external</span>';
 return '';
}
...
var PVSL_DDT='profile';      // profile | solfin | strisk | lilly | reqs
...
var PVVENDOR_COLORS=['#0F3A85','#6A4C93','#A6541C','#3C7A78','#8A5A00'];
...
function pvPlaneDot(a,fh,rh,selId){
 // Marc: 4 EQUAL quadrants, the cutoff maps to the CENTER (50%). Piecewise scale: [0..cutoff]→[pad..50],
 // [cutoff..max]→[50..100-pad], so a supplier sits on the correct side of a centered divider.
 // #4 (Marc): dots are clickable and tie to the differentiators accordion (pvSegPick); selId rings the open one.
 fh=(fh!=null?fh:60);rh=(rh!=null?rh:2.5);
 var rv=pvAuthRisk(a.id,a.riskScore);
 var f=Math.max(0,Math.min(100,a.fitScore));
 var rvv=Math.max(0,Math.min(5,rv));
 var left=pvRound(f<=fh?(6+(f/(fh||1))*44):(50+((f-fh)/((100-fh)||1))*44),2);
 var top=pvRound(rvv<=rh?(8+(rvv/(rh||1))*42):(50+((rvv-rh)/((5-rh)||1))*42),2);
 var dq=a.segment==='disqualified';var col=pvSupColor(a);
 var title=a.name+' · fit '+a.fitScore+'/100 · risk '+rv+'/5 · '+PVSEG_LBL[a.segment]+(a.incumbent?' · incumbent':'');
 var selCls=(selId!=null&&a.id===selId)?' pdot-sel':'';
 return '<span class="pdot'+(dq?' dq':'')+(a.incumbent?' inc':'')+selCls+'" data-id="'+escD(a.id)+'" onclick="pvSegPick(\''+jarg(a.id)+'\')" style="left:'+left+'%;top:'+top+'%;'+(dq?'':'background:'+col+';border-color:'+col)+'" title="'+escD(title)+'">'+escD(pvInitials(a.name))+'</span>';
}
...
function pvHeatmapHtml(refl,opts){
 ...
 var frDiff=diffCats.length?diffCats.map(function(c){return frPill(escD(c.label)+' &nbsp;&Delta;'+c.spread,'var(--ti-blue)','#0F3A85');}).join(''):'<span style="font-size:12px;color:var(--mut2)">The field is tightly bunched across every category.</span>';
 ...
 var mustTrail=[];mustSubs.forEach(function(m){elig.forEach(function(a){var cand=candOf[a.id];var v=(cand&&cand.subFit&&cand.subFit[m.catId])?cand.subFit[m.catId][m.subId]:null;if(v!=null&&v<GAP)mustTrail.push({who:a.name,sub:m.sub,v:pvRound(v,1)});});});
 var mustNames=mustSubs.map(function(m){return escD(m.sub);}).join(', ');
 var mustBody=mustTrail.length?mustTrail.map(function(t){return '<div style="font-size:12px;line-height:1.5;margin-bottom:3px"><span style="color:var(--riskred);font-weight:700">&#9888;</span> <b>'+escD(t.who)+'</b> trails on the must-have <b>'+escD(t.sub)+'</b> ('+t.v+'/5), a knockout risk even at a strong overall fit.</div>';}).join(''):'<div style="font-size:12px;color:var(--mut);line-height:1.5">Every eligible vendor clears all must-have requirements at 3.5+/5.</div>';
 var mustWatch=mustSubs.length?('<div style="margin-top:16px;padding-top:14px;border-top:1px solid var(--line)"><div style="font:700 9px var(--mono);text-transform:uppercase;letter-spacing:.05em;color:var(--mut2);margin-bottom:8px">Must-have watch <span style="color:var(--mut2);font-weight:400;text-transform:none;letter-spacing:0">&middot; knockout requirements: '+mustNames+'</span></div>'+mustBody+'</div>'):'';
 ...
 var fieldRead='<div class="sa-card"><div class="card-hd"><svg viewBox="0 0 24 24"><path d="M3 12h4l3 8 4-16 3 8h4"/></svg><span class="ct">Where the decision is made</span></div><div class="scc-b"><p class="fr-lede">'+frLede+'</p>'+frTable+mustWatch+'</div></div>';
 ...
 return '<div class="sa-card">'+
   ...
   '<div class="scc-b">'+leadNarr+table+legend+rationale+
   '</div></div>'+fieldRead;
}
...
function pvHmRationaleHtml(ordered,reqs,covOf,candOf,hm){
 ...
 var rows=reqs.map(function(r){
   ...
   return '<div class="vratrow"><div class="vrathd"><span class="hcell" style="'+pvHmRamp(sc)+'">'+escD(sc)+'</span><span class="vratlbl">'+escD(r.label)+'</span>'+(lead?' <span class="flag inc" style="margin:0">leads</span>':'')+covchip+'</div><div class="vratnar">'+escD(narr[r.id]||'No category rationale on file.')+'</div>'+sub+'</div>';
 }).join('');
...
function pvRiskHtml(refl){
 ...
 var candOf={};ordered.forEach(function(a){var cn=pvCandById(a.id);candOf[a.id]=cn;if(cn){if(!cn.risk)cn.risk={};if(cn.risk.esg==null&&PVSL_ESG[cn.id]!=null)cn.risk.esg=PVSL_ESG[cn.id];if(!cn.riskNarr)cn.riskNarr={};if(!cn.riskNarr.esg)cn.riskNarr.esg=PVSL_ESG_NARR[cn.id]||'ESG & regulatory posture, illustrative external read; confirm in due diligence.';}});
 ...
 var finLine='';
 if(finPairs.length>=2){var fs=finPairs.slice().sort(function(x,y){return x.v-y.v;});finLine=' Financial stability reads on an illustrative, externally-sourced basis (credible public sources · not validated), strongest for '+escD(fs[0].name)+' and thinnest for '+escD(fs[fs.length-1].name)+'.';}
 else if(finPairs.length===1){finLine=' Financial stability reads on an illustrative, externally-sourced basis (credible public sources · not validated).';}
 ...
 return '<div class="sa-card">'+
   ...
   '<div class="scc-b">'+cc+table+legend+posture+rationale+
   '</div></div>';
}
...
function pvRkRationaleHtml(ordered,dims,candOf){
 ...
 var rows=dims.map(function(d){
   ...
   return '<div class="vratrow"><div class="vrathd"><span class="rcell" style="'+(sc==null?'background:var(--bg);color:var(--mut2)':pvRiskBg(sc))+'">'+(sc==null?'&ndash;':escD(sc))+'</span><span class="vratlbl">'+escD(d.label)+'</span></div><div class="vratnar">'+escD(narr[d.id]||'No dimension rationale on file.')+'</div>'+sub+'</div>';
 }).join('');
...
function pvRiskPostureHtml(a,cand){
 ...
 if(!a||(!rp.breach&&!rp.certs&&!rp.rating&&!rp.regulatory))return '';
 ...
 return '<div class="riskpost" style="--rpacc:'+acc+'"><div class="riskpost-h"><span class="riskpost-dot" style="background:'+acc+'"></span>'+escD(a.name)+' &middot; risk posture <span class="riskpost-tag">illustrative &middot; reflect-only</span></div><div class="riskpost-grid">'+
...
function pvDeepDiveHtml(id,refl,input){
 ...
 h+='<div class="sa-card">'+pvDdCardHd('<path d="M20 7l-8-4-8 4 8 4 8-4z"/><path d="M4 7v6l8 4 8-4V7"/>','<span class="ct">'+escD(a.name)+' · profile &amp; fit</span>')+
   '<div class="scc-b"><div class="statgrid" style="grid-template-columns:repeat(5,1fr)">'+
     ...
     '<div class="statbox"><div class="sb-l">Rank</div><div class="sb-v">'+(a.rank!=null?('#'+escD(a.rank)):'&mdash;')+'</div></div>'+
   '</div>'+
 ...
 h+='<div class="sa-card">'+pvDdCardHd('<path d="M12 3l7 3v5c0 5-3.5 8-7 9-3.5-1-7-4-7-9V6z"/>','<span class="ct">Strengths &amp; Risks</span>')+
   ...
   '<div class="footbound">Narrative risks are reflect-only; a hard flag disqualifies, a soft flag is recorded for review. Dimension scores roll up from the sub-factors on the Risk Assessment subtab.</div></div></div>';
...
function pvExecSummaryHtml(refl,input){
 var L=refl.landscape,rec=L.recommendation,lead=rec.lead;
 var ranked=L.assessments.filter(function(a){return a.rank!=null;}).sort(function(x,y){return x.rank-y.rank;});
 var f5=function(o){return pvRound((o.fitScore||0)/20,1);};
 ...
 var leadElev=lead?(lead.riskScore>=riskHigh):false;
 ...
 var h='<div class="exechd"><h2>Executive Summary</h2><div class="spacer"></div><button class="rfxbtn" onclick="pvStartRfx()"><svg viewBox="0 0 24 24"><path d="M5 12h14M13 6l6 6-6 6"/></svg>Start an RFx</button></div>';
 // Pass 2B: the Start-an-RFx flow now opens as a DRAWER (pvStartRfx), no persistent picker at the top.
 // Compact recommendation card: top candidate + secondary, a one-line rationale each (the full interactive slate lives only in the drawer).
 ...
 var recrat=lead?(escD(lead.name)+' ranks first on the weighted 6-category model ('+escD(f5(lead))+'/5) with '+(leadElev?'elevated':'contained')+' risk ('+escD(lead.riskScore)+'/5). Advance it as the RFx pace-setter; open <b>Start an RFx</b> for the full competitive slate and a value lever.'):'';
 // Recommendation, flat standings card (design handoff): header + all-vendor standings, rank-1 raised, next-action band, advisory note.
 ...
 var recRows=ranked.map(function(a,i){
   var isLead=a.rank===1;var last=(i===ranked.length-1);
   var cls=isLead?'rgrid lead':('rgrid run'+(last?' last':''));
   var ra=recAuth[a.id];
 ...
 // G15 shortlist band, the explicit "advance these" line: eligible vendors within a competitive band of the top composite.
 ...
 var slBand=(lead&&shortlist.length)?('<div style="margin:2px 12px 10px;padding:9px 12px;border-radius:8px;background:var(--ti-blue);border-left:3px solid var(--navy);font-size:12px;line-height:1.5"><b style="color:var(--navy)">Shortlist to advance:</b> '+shortlist.map(function(x){return escD(x.a.name);}).join(', ')+' <span style="color:var(--mut2)">&mdash; within '+slBandPts+' composite points of the top ('+topComp+'); the rest of the field falls outside the competitive band.</span></div>'):'';
 var recHtml='<div class="pvrec">'+
   '<div class="rh"><span class="rt">Recommendation</span></div>'+
   (lead
     ? ('<div class="rtbl">'+recHead+recRows+'</div>'+slBand)
     : ('<div class="rtbl">'+recHead+'<div class="rgrid run last"><span></span><span class="rnm">No candidate is eligible; every vendor carries a hard flag. Re-scope the requirements or widen the field.</span><span></span><span></span><span></span><span></span></div></div>'))+
 '</div>';
 ...
 // Est. 3-yr TCO folds into the Evaluation-summary prose below.
 ...
 if(ranked.length){
   ...
   prose+='The reads are reflect-only, weighted across '+sumSubs+' sub-requirements and a five-dimension risk model drawn from credible public sources; nothing is scored, contacted, or awarded here.';
 ...
 var evalHtml='<div class="sa-card"><div class="card-hd"><svg viewBox="0 0 24 24"><path d="M4 5h16M4 10h16M4 15h10"/></svg><span class="ct">Evaluation Summary</span></div><div class="scc-b"><div class="pvlede" style="margin:0">'+escD(prose)+'</div>'+evalFindings+'</div></div>';
...
function pvRankBarHtml(refl,input){
 ...
 var rows=ranked.map(function(a){
   var f5=pvRound(a.fitScore/20,1);var w=pvRound(f5/5*100,1);var col=colorOf[a.id]||'#0F3A85';
...
function pvSegPlaneHtml(refl,input){
 var L=refl.landscape;
 ...
 var dots=L.assessments.map(function(a){return pvPlaneDot(a,fitHigh,riskHigh,selId);}).join('');
 var plane='<div class="plane"><div class="pq pq-niche"></div><div class="pq pq-leader"></div><div class="pq pq-caution"></div><div class="pq pq-chal"></div>'+
   '<div class="pdiv-v" style="left:50%"></div><div class="pdiv-h" style="top:50%"></div>'+
   ...
   '<div class="axlab" style="left:8px;bottom:50%">&#8593; lower risk</div><div class="axlab" style="left:50%;transform:translateX(-50%);bottom:2px">higher fit &#8594;</div>'+dots+'</div>';
 // plane key, segment colours present + the incumbent marker (inline; mirrors the .pdot.inc badge)
 var segsPresent=PVSEG_ORDER.filter(function(s){return L.assessments.some(function(a){return a.segment===s;});});
 ...
 var incKey=L.assessments.some(function(a){return a.incumbent;})?'<span style="display:inline-flex;align-items:center;gap:5px"><span style="position:relative;width:11px;height:11px;border-radius:50%;background:'+PVSEG_COLOR.leader+';flex:none"><span style="position:absolute;right:-3px;top:-3px;width:7px;height:7px;border-radius:50%;background:var(--surface);border:1.5px solid var(--blue)"></span></span>Incumbent</span>':'';
 ...
 return '<div class="sa-card"><div class="card-hd"><svg viewBox="0 0 24 24"><circle cx="11" cy="11" r="7"/><path d="M21 21l-4.3-4.3"/></svg><span class="ct">Segmentation &amp; Differentiators</span><span class="cs">'+escD(ranked.length)+' ranked · fit cut '+escD(fitHigh)+'/100 · risk cut '+escD(riskHigh)+'/5</span></div>'+
   '<div class="scc-b"><div class="execgrid seg-md"><div class="segL">'+plane+legend+assess+'</div>'+accHtml+'</div></div></div>';
}
...
function pvRfxRenderDrawer(){
 ...
 d.innerHTML='<div class="pvsl"><div class="dh"><div><h3>Start an RFx</h3><div class="dsub">Draft bidder set · reflect-only until the rep approves</div></div><div class="dc" onclick="pvCloseRfx()">&times;</div></div><div class="db">'+pvRfxBodyHtml(refl,PVSL_INPUT)+'</div></div>';
...
function pvRfxBodyHtml(refl,input){
 ...
 return '<p class="rfxlede">The recommended slate is pre-checked. Add or remove suppliers, pull in a qualified incumbent, or enter one off the landscape, then route the draft to the sourcing rep for approval. Reflect-only until approved; on approval it seeds the RFx tab bidder set.</p>'+
...
function pvDDSection(ddt,a,cand,refl,input){
 var dd=cand.deepDive||{};
 var pvCard=function(icon,title,inner){return '<div class="sa-card">'+pvDdCardHd(icon,'<span class="ct">'+title+'</span>')+'<div class="scc-b">'+inner+'</div></div>';};
 if(ddt==='profile'){
   // Key attributes, two-column zebra key/value list (design handoff): one blue key colour, a single
   // purple dagger marks internal/negotiation facts, one footnote instead of a per-value provenance tag.
   var kaPairs=[[['HQ','hq'],['Founded','founded']],[['Financial','financial'],['Gartner','gartner']],[['Pricing','pricing'],['Contract','contractFlex']],[['Integration','integration'],['ESG','esg']]];
   var kaCell=function(d){var val=(dd.attrs&&dd.attrs[d[1]]!=null&&dd.attrs[d[1]]!=='')?dd.attrs[d[1]]:'Data not available';var internal=!!(dd.attrsSrc&&dd.attrsSrc[d[1]]==='int');return '<span class="kk">'+escD(d[0])+'</span><span class="kvv">'+escD(val)+(internal?'<sup>&dagger;</sup>':'')+'</span>';};
   var kaHasInternal=Object.keys((dd.attrsSrc||{})).some(function(k){return dd.attrsSrc[k]==='int';});
   var kaRows=kaPairs.map(function(pair,r){return '<div class="kar'+(r%2===0?' z':'')+'">'+kaCell(pair[0])+kaCell(pair[1])+'</div>';}).join('');
   var kaHtml='<div class="pvka">'+kaRows+'</div>'+(kaHasInternal?'<p class="pvka-note"><b>&dagger;</b> Internal / negotiation data, all other facts are from external public sources.</p>':'');
   ...
   if(dd.clients||dd.relationship){
     ...
     extIntBlk='<div class="subt" style="margin-top:14px">Market Presence &amp; History</div><div style="border-top:1px solid var(--line)">'+
   ...
   var gartnerV=(dd.attrs&&dd.attrs.gartner)?dd.attrs.gartner:'';
   var roadBlk=(roadScore!=null||roadNarr||gartnerV)?('<div class="subt" style="margin-top:14px">Roadmap &amp; vision '+pvSrcTag('ext')+'</div><div style="border-top:1px solid var(--line);padding-top:9px">'+
     (gartnerV?'<div style="font-size:12.5px;margin-bottom:7px"><b style="color:var(--mut2)">Analyst position &middot;</b> '+escD(gartnerV)+'</div>':'')+
     (roadScore!=null?'<div style="display:flex;align-items:center;gap:9px;margin-bottom:7px"><span style="font-size:11.5px;color:var(--mut2);font-weight:600;width:150px">Roadmap &amp; extensibility fit</span><div style="flex:1;max-width:170px;height:7px;border-radius:4px;background:var(--nested);overflow:hidden"><i style="display:block;height:100%;width:'+(roadScore/5*100)+'%;background:var(--ddacc,#0F3A85)"></i></div><span style="font-family:var(--mono);font-weight:700;font-size:12px;color:var(--ddacc,var(--navy))">'+escD(roadScore)+'/5</span></div>':'')+
     (roadNarr?'<div style="font-size:12.5px;color:var(--mut);line-height:1.5">'+escD(roadNarr)+'</div>':'')+
     '</div>'):'';
   // Identity block (skill Section 1), resolve WHO the supplier legally is. Illustrative reflect-only
   // enrichment; an identity-confidence chip flags Confirmed vs Best-guess (a demo of the G2 identity gate).
   var idn=dd.identity||{};
   var idKnown=idn.legal||idn.parent||idn.ownership||idn.ticker||idn.jurisdiction;
   var idBlk='';
   if(idKnown){
     var confMeta=idn.confidence==='Confirmed'?{c:'#0F3A85',bg:'var(--ti-blue)'}:{c:'#8A5A00',bg:'var(--ti-amber)'};
     var confChip=idn.confidence?' <span style="font:700 9px var(--mono);text-transform:uppercase;letter-spacing:.03em;padding:2px 9px;border-radius:20px;color:'+confMeta.c+';background:'+confMeta.bg+';vertical-align:1px">Identity: '+escD(idn.confidence)+'</span>':'';
     var idPairs=[[['Legal entity',idn.legal],['Parent / owner',idn.parent]],[['Ownership',idn.ownership],['Ticker',idn.ticker||'n/a · private']],[['HQ',(dd.attrs&&dd.attrs.hq)||''],['Jurisdiction',idn.jurisdiction]]];
     var idCell=function(d){var v=(d[1]!=null&&d[1]!=='')?d[1]:'Data not available';return '<span class="kk">'+escD(d[0])+'</span><span class="kvv">'+escD(v)+'</span>';};
     var idRows=idPairs.map(function(pair,r){return '<div class="kar'+(r%2===0?' z':'')+'">'+idCell(pair[0])+idCell(pair[1])+'</div>';}).join('');
     idBlk='<div class="subt" style="margin-top:16px">Corporate identity '+pvSrcTag('ext')+confChip+'</div><div class="pvka">'+idRows+'</div><p class="pvka-note">Legal entity, parent and ownership are <b>illustrative</b> reflect-only enrichment (credible public sources &middot; not validated); a live deep dive resolves identity against the Lilly vendor master before asserting a parent, a wrong-company profile is worse than none.</p>';
   }
   ...
   if(offs.length||idn.delivery||dd.solution){
     var offRows=offs.map(function(o){return '<tr><td class="dt">'+escD(o.name||'')+'</td><td class="dd">'+escD(o.note||'')+'</td></tr>';}).join('');
     offBlk='<div class="subt" style="margin-top:16px">Offering profile <span style="font-weight:500;color:var(--mut2);font-size:11px;text-transform:none;letter-spacing:0">&middot; illustrative</span></div>'+
   ...
   // #.2 (Marc): real-vendor "Company snapshot", headcount / customers / valuation / funding as stat tiles, then
   // footprint / leadership / partners / references. Enriches the fictional supplier to a Databricks-level dossier.
   var comp=dd.company||{};var compBlk='';
   if(comp.headcount||comp.customers||comp.valuation||comp.funding){
     var coT=function(v,l){return v?'<div class="cosnap-t"><div class="cosnap-v">'+escD(v)+'</div><div class="cosnap-l">'+escD(l)+'</div></div>':'';};
     var coLine=function(k,v){return v?'<div class="cosnap-line"><span class="cosnap-k">'+escD(k)+'</span> '+escD(v)+'</div>':'';};
     compBlk='<div class="subt" style="margin-top:16px">Company snapshot '+pvSrcTag('ext')+'</div><div class="cosnap">'+coT(comp.headcount,'Headcount')+coT(comp.customers,'Customers')+coT(comp.valuation,'Valuation')+coT(comp.funding,'Funding raised')+'</div>'+coLine('Founded',comp.founded)+coLine('Footprint',comp.footprint)+coLine('Leadership',comp.leadership)+coLine('Partners & ecosystem',comp.partners)+coLine('Named references',comp.refs);
   }
   return pvCard('<path d="M20 7l-8-4-8 4 8 4 8-4z"/><path d="M4 7v6l8 4 8-4V7"/>',escD(a.name)+' &middot; profile','<div class="pvlede">'+escD(dd.overview||'')+'</div><div class="pvlede"><b style="color:var(--emph)">Why this vendor for Lilly.</b> '+escD(dd.whyLilly||'')+'</div>'+compBlk+idBlk+offBlk+'<div class="subt" style="margin-top:16px">Key Attributes</div>'+kaHtml+roadBlk+extIntBlk);
 }
 if(ddt==='solfin'){
   var fin=cand.financials||{};
   // Pass 2B: one card; financial figures as a definition list (was a 6-card grid)
   var finRows=[['Revenue','revenue'],['Growth','growth'],['Margin / EBITDA','margin'],['ARR','arr'],['Cash / runway','cash'],['Guidance','guidance']].map(function(fd){return '<tr><td class="dt">'+escD(fd[0])+'</td><td class="dd">'+escD(fin[fd[1]]||'Data not available')+'</td></tr>';}).join('');
   ...
   var pricePosCol=cmFit==null?'var(--mut2)':cmFit>=4.5?'#0F3A85':cmFit>=3.5?'#2E5E8C':'#8A5A00';
   var lockin=(cand.risk&&cand.risk.lockin!=null)?cand.risk.lockin:null;
   var swLbl=lockin==null?'—':lockin>=3?'High':lockin>=2?'Medium':'Low';
   var swCol=lockin==null?'var(--mut2)':lockin>=3?'#A23A30':lockin>=2?'var(--amber-d)':'#0F3A85';
   ...
   var viab=finRisk==null?null:(finRisk<1.75?{l:'Safe',c:'#0F3A85',bg:'var(--ti-blue)'}:finRisk<3?{l:'Watch',c:'#8A5A00',bg:'var(--ti-amber)'}:{l:'Distress risk',c:'#A23A30',bg:'var(--ti-red)'});
   ...
   var mpBlk=mpGartner?('<div class="subt">Market position '+pvSrcTag('ext')+'</div><div class="pvlede" style="margin:0 0 4px"><b style="color:var(--mut2)">Analyst position &middot;</b> '+escD(mpGartner)+'</div>'):'';
   ...
   var newsBlk=newsRows?('<div class="subt" style="margin-top:16px">Recent news <span style="font-weight:500;color:var(--mut2);font-size:11px;text-transform:none;letter-spacing:0">&middot; illustrative, last ~18 months</span></div><div style="overflow-x:auto"><table class="pvdl"><tbody>'+newsRows+'</tbody></table></div>'):'';
   return pvCard('<path d="M3 3v18h18"/><path d="M7 14l3-3 3 3 5-6"/>','Market &amp; financials',
     mpBlk+solPtr+
     '<div class="subt" style="margin-top:16px">Financial health'+viabBadge+'</div><div class="pvlede mut">'+escD(dd.finHealth||'')+'</div>'+
     (viab?'<div class="footbound" style="margin-top:4px">Viability grade read from the financial-stability signal (runway, margin, scale, standard distress signals): <b style="color:var(--navy)">Safe</b> below 1.75 &middot; <b style="color:var(--amber-d)">Watch</b> 1.75&ndash;3 &middot; <b style="color:var(--riskred)">Distress risk</b> at 3+/5. Reflect-only, illustrative.</div>':'')+
     '<div class="subt" style="margin-top:14px">Reported figures '+pvSrcTag('ext')+'</div><div style="overflow-x:auto"><table class="pvdl"><tbody>'+finRows+'</tbody></table></div>'+
     '<div class="subt" style="margin-top:16px">Commercial estimate <span style="font-weight:500;color:var(--mut2);font-size:11px;text-transform:none;letter-spacing:0">&middot; illustrative</span></div><div style="overflow-x:auto"><table class="pvdl"><tbody>'+ceRows+'</tbody></table></div>'+
     newsBlk+
     '<div class="footbound"><b>Illustrative mock, tagged external.</b> Reported figures are owner-sanctioned mock enrichment from credible public sources (not validated). The commercial estimate is a light read to spot <b>price-led vs premium</b> and <b>lock-in / switching exposure</b>, the firm 3-year TCO comes from the RFx bids (normalized pricing) and the Deal pro-forma, and exit terms are settled in negotiation. A genuinely undisclosed figure would read Data not available, never silently invented.</div>');
 }
 if(ddt==='strisk'){
   ...
   var dimRows=dims.map(function(dm){var sc=rawRisk[dm.id];var col=sc==null?'var(--mut2)':sc>=3?'#A23A30':sc>=2?'var(--amber-d)':'#0F3A85';var w=sc==null?0:Math.min(100,sc/5*100);return '<tr><td style="text-align:left;font-weight:600">'+escD(dm.label)+'</td><td style="text-align:right;font-weight:700;color:'+col+';white-space:nowrap">'+(sc==null?'&mdash;':escD(sc)+' / 5')+'</td><td style="width:130px"><div style="height:7px;border-radius:4px;background:var(--line);overflow:hidden"><i style="display:block;height:100%;width:'+w+'%;background:'+col+'"></i></div></td></tr>';}).join('');
   var com=dd.commercial||{};
   var contractBlk=com.contracting?('<div class="subt" style="margin-top:16px">Contracting flexibility</div><div class="pvlede" style="margin:0">'+escD(com.contracting)+'</div>'):'';
   var regBlk=com.regulatory?('<div class="subt" style="margin-top:14px">Regulatory / GxP</div><div class="pvlede" style="margin:0">'+escD(com.regulatory)+'</div>'):'';
   return pvCard('<path d="M12 3l7 3v5c0 5-3.5 8-7 9-3.5-1-7-4-7-9V6z"/>','Strengths &amp; Risks',
     '<div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(260px,1fr));gap:24px"><div><div class="subt">Strengths</div>'+strengths+'</div><div><div class="subt">Risks</div>'+risks+'</div></div>'+
     contractBlk+regBlk+
     '<div class="subt" style="margin-top:16px">Risk Dimensions <span style="font-weight:500;color:var(--mut2);font-size:11px;text-transform:none;letter-spacing:0">&middot; 0&ndash;5, higher is worse</span></div>'+
     '<div class="mxwrap"><table class="mx" style="width:100%"><tbody>'+dimRows+'</tbody></table></div>'+
     '<div class="footbound">Narrative risks are reflect-only; a hard flag disqualifies, a soft flag is recorded for review. Dimension scores roll up from the sub-factors on the Risk Assessment subtab.</div>');
 }
 if(ddt==='lilly'){
   ...
   var capR=a.fitScore>=80?{l:'High',c:'#0F3A85',bg:'var(--ti-blue)'}:a.fitScore>=65?{l:'Medium',c:'#8A5A00',bg:'var(--ti-amber)'}:{l:'Low',c:'#A23A30',bg:'var(--ti-red)'};
   var rel=lf.relation||((dd.relationship&&/incumbent/i.test(dd.relationship))?'Active incumbent':'Net new');
   var relMeta=rel==='Active incumbent'?{c:'#0F3A85',bg:'var(--ti-blue)'}:rel==='Prior engagement'?{c:'#2E5E8C',bg:'var(--ti-blue)'}:{c:'var(--mut2)',bg:'var(--nested)'};
   var sf=(lf.strategic||'').toLowerCase();
   var sfMeta=sf==='supports'?{l:'Supports',c:'#0F3A85',bg:'var(--ti-blue)'}:sf==='works-against'?{l:'Works against',c:'#A23A30',bg:'var(--ti-red)'}:sf?{l:'Neutral',c:'#8A5A00',bg:'var(--ti-amber)'}:{l:'Data not available',c:'var(--mut2)',bg:'var(--nested)'};
   ...
   var pharmaBlk=lf.pharma?('<div class="subt" style="margin-top:16px">Pharma / GxP gates <span style="font-weight:500;color:var(--mut2);font-size:11px;text-transform:none;letter-spacing:0">&middot; risk signal, not a routed screen</span></div><div class="pvlede" style="margin:0">'+escD(lf.pharma)+'</div>'):'';
   var valueBlk=lf.value?('<div class="subt" style="margin-top:16px">Value at risk / next move</div><div class="pvlede" style="margin:0">'+escD(lf.value)+'</div>'):'';
   var lfCap='<div class="leadnarr"><b>Lilly fit, synthesis.</b> A qualitative read of how this supplier fits Lilly, distinct from the scored <b>Requirements Fit</b> heatmap. Reflect-only and illustrative; no vendor is selected, contacted or awarded, and any gate below is a <b>risk signal</b> to clear downstream (RFx / onboarding / contracting), never an SME routing on the landscape. '+infoHover('Requirements Fit scores capability against each requirement. Lilly Fit is the synthesis judgment: relationship, strategic fit, pharma gates and value-at-risk. Both are reflect-only landscape estimates built without contacting the supplier; actual SME routing happens downstream once an RFx or onboarding begins.',{aria:'Lilly Fit vs Requirements Fit'})+'</div>';
   return pvCard('<circle cx="12" cy="12" r="8"/><circle cx="12" cy="12" r="3"/>',escD(a.name)+' &middot; Lilly fit',
     lfCap+
     '<div class="pvlede"><b style="color:var(--emph)">Why this vendor for Lilly.</b> '+escD(dd.whyLilly||'')+'</div>'+
     lfPills+
     pharmaBlk+valueBlk+
     '<div class="footbound">Capability-fit rating is derived from the weighted requirements fit ('+escD(f5)+'/5). Relationship, strategic fit, pharma gates and next-move are illustrative reflect-only reads, the authoritative relationship + TPRM status live in Supplier 360 and the downstream RFx / onboarding record.</div>');
 }
 if(ddt==='reqs'){
   ...
   return pvHeatmapHtml(refl,{inDeep:true});
 }
...
function pvCompPositionHtml(a,cand,refl,input){
 ...
 var sub=(a.rank!=null?('#'+a.rank+' of '+L.eligibleCount):'disqualified')+' · '+escD(PVSEG_LBL[a.segment]||a.segment);
 var shortCat=function(s){return String(s).split(/\s*[&·]\s*/)[0].trim();};
 var leadsHtml=leads.length?leads.slice(0,3).map(function(l){return '<span class="ddcp-mgc" style="color:'+acc+';border-color:'+acc+'66">'+escD(shortCat(l.label))+' +'+escD(l.margin)+'</span>';}).join(''):'<span style="font-size:11.5px;color:var(--mut2)">&mdash; leads no single category</span>';
 var strongHtml=best?(escD(reqLabel[best.requirementId]||best.requirementId)+' '+escD(pvRound(best.score,2))):'&mdash;';
 ...
 return '<div class="ddcp">'+
   '<div class="ddcp-mast">'+
     '<div class="ddcp-badge" style="background:'+acc+'"><span class="vv">'+escD(vm.lbl)+'</span><span class="rr">'+sub+'</span></div>'+
...
function pvDeepDiveTabHtml(refl,input){
 ...
 return '<div class="dd" '+ddacc+'>'+bar+pvCompPositionHtml(a,cand,refl,input)+tabbar+pvDDSection(ddt,a,cand,refl,input)+'</div>';
}
...
function landscapeThinHTML(){
 ...
 let h='<p class="dashintro"><b>Supplier landscape</b>, candidate suppliers assessed for fit. This is a reflect-only read built from <b>credible public sources</b>; nothing here is validated or a recommendation.</p>';
