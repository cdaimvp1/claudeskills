...
- **Suite-wide guardrails note:** The shared Execution Guardrails G1-G13 (tool selection, gate checks, existing-context-first, definition tracing, data-model-first for dashboards, pre-delivery self-test, research minimums, pass-artifact enforcement, anti-collapse) apply to every run of this skill. This is a cross-cutting suite convention, not a per-skill version; see the GLOBAL OPERATING RULES (Rule 9) above for the full G1-G13 text.
