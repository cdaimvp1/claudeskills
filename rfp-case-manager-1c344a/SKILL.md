...
Mirror of the rfp-engine case-handoff-schema reference (its own companion file). Schema for `case_handoff.json`, generated by rfp-engine when the user selects case handoff and consumed by rfp-case-manager to seed the case file. (Note: rfp-engine's source copy still describes legacy provisioning actions on receipt; under the v2.0 "no provisioning spec" decision, rfp-case-manager does NOT create SharePoint folders or Teams sites on receipt: it adapts to whatever exists and builds the case file.)
...
```json
{
  ...
  "artifacts": {
    ...
    "files": [
      {
        ...
        "type": "RFP_Instructions | Requirements_Matrix | Schedule | Pricing_Template | Demo_Evaluation_Guide | Post_Award_Timeline | Stakeholder_Roster",
    ...
    "invitation_email": {
      ...
      "body": "string - email body with [Vendor Name] / [Vendor Contact] placeholders",
      "delivery_method": "actionable draft in chat (no auto-send; message_compose unavailable falls back to a labeled inline draft)",
...
**Validation on receipt** (reject or flag, never silently accept bad data):

| Check | Rule |
|-------|------|
| `schema_version` | Must be "1.0" |
| `case.case_id` | Must be unique; if it collides with an existing case, ask the user before merging |
| `schedule.proposals_due` | Must be after `schedule.qa_deadline` |
| `suppliers` | At least 1 supplier present |
| `stakeholders` | At least 1 with `approval_authority = true`, else flag NEEDS_INPUT |

**Actions on receipt (v2.0 behavior).** Map the handoff into a new `_case_file.json` (Case File Schema above). Do NOT create SharePoint folders, Teams sites, or channels: if a Team is bound, crawl what already exists and map artifact paths onto the inferred folder semantics; if no Team is bound, hold artifact paths as references and ingest any uploaded copies. Confirm the seeded case file with the user in one pass.
