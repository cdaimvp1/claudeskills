...
**A2 FINDING (Marc 2026-07-26, reviewing in Edge):** the current Landscape build's SUPPLIER DEEP-DIVE looks changed from what Marc approved, and the CROSS-SUPPLIER VIEW seems missing/buried. NOT changed this session — the delivered build is byte-identical to the `_platform_build` engine (same md5 on pv-07b-deepdive.js / pv-07-landscape-render.js). The cross-supplier heatmap IS still in the code (deep-dive "Portfolio summary" -> semantic cross-supplier heatmap). Likely a regression from the earlier Deep-Dive redesign v3 / R2 review. TODO: with Marc, compare against the earlier build he approved (e.g. `_dashboard_previews/supplier-landscape-REDESIGN.html`, `_canonical_originals/supplier-landscape-ORIGINAL.html`), restore/re-prominence the cross-supplier view, reconcile the deep-dive layout. THIS is the substance of A2.
